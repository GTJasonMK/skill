...
Standard-library only. Input should contain one row per asset-date return and
...
from quant_utils import is_missing, mean, parse_float, read_dataframe, require_columns, sorted_group_keys, summarize_values
...
def build_report(
    ...
    events = []
    abnormal_by_offset: dict[int, list[float]] = {offset: [] for offset in range(-pre_window, post_window + 1)}
    skipped_events = 0
    for asset in sorted_group_keys(list(grouped)):
        ...
        for event_index, item in enumerate(asset_rows):
            if not item["event"]:
                continue
            expected = estimate_expected_return(asset_rows, event_index, estimation_window, estimation_gap, benchmark_return_col)
            if expected is None:
                skipped_events += 1
                continue
            ar_path = []
            ...
            for offset in range(-pre_window, post_window + 1):
                ...
                if idx < 0 or idx >= len(asset_rows):
                    ...
                    continue
                obs = asset_rows[idx]
                abnormal = obs["return"] - obs["benchmark_return"] if benchmark_return_col else obs["return"] - expected
                car += abnormal
                abnormal_by_offset[offset].append(abnormal)
                ar_path.append({"offset": offset, "date": obs["date"], "abnormal_return": abnormal, "car_to_offset": car})
            events.append(
                {
                    "asset": asset,
    ...
    return {
        ...
        "notes": [
            ...
            "Without a benchmark, expected return is the asset's mean return from the pre-event estimation window.",
            "Check overlapping events, event-time leakage, confounding news, clustering, and multiple testing before making claims.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
        f"| All events | {car['n']} | {car['mean']} | {car['stdev']} | {car['t_stat']} | {car['positive_rate']} | {car['min']} | {car['max']} |",
        f"| Complete windows | {complete['n']} | {complete['mean']} | {complete['stdev']} | {complete['t_stat']} | {complete['positive_rate']} | {complete['min']} | {complete['max']} |",
        "",
    ...
    for item in report["aggregate_abnormal_returns"]:
        lines.append(f"| {item['offset']} | {item['n']} | {item['mean']} | {item['t_stat']} | {item['positive_rate']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compute simple event-study abnormal return diagnostics from a long CSV.")
    ...
    require_columns(
        header,
        [args.date_col, args.asset_col, args.return_col, args.event_col] + ([args.benchmark_return_col] if args.benchmark_return_col else []),
    )
    report = build_report(
        rows,
        args.date_col,
