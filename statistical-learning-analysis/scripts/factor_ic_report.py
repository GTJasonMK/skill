...
def build_report(
    ...
    by_date, dropped = period_ics(df, date_col, factor_col, forward_return_col, min_assets)
    ...
    return {
        ...
        "notes": [
            ...
            "Rank IC is the per-date Spearman correlation and is usually more robust for cross-sectional factor research.",
            "Rows must already respect point-in-time signal availability and the intended forward-return horizon.",
        ],
    }
...
def main() -> int:
    ...
    report = build_report(df, args.date_col, args.factor_col, args.forward_return_col, args.min_assets_per_date)
    if args.output_json:
