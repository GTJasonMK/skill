...
Standard-library only. Input is a checklist CSV with category, check, status,
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Category Counts", "", "| Category | " + " | ".join(sorted(report["status_counts"])) + " |", "| --- | " + " | ".join("---" for _ in sorted(report["status_counts"])) + " |"])
    ...
    for category in sorted_group_keys(list(report["category_counts"])):
        ...
        lines.append("| " + category + " | " + " | ".join(str(counts.get(status, 0)) for status in statuses) + " |")
    lines.extend(["", "## Blockers", "", "| Severity | Category | Check | Status | Owner | Evidence |", "| --- | --- | --- | --- | --- | --- |"])
    for item in report["blockers"]:
        lines.append(f"| {item['severity']} | {item['category']} | {item['check']} | {item['status']} | {item['owner']} | {item['evidence']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    ...
    report = build_report(rows, args.category_col, args.check_col, args.status_col, args.severity_col, args.evidence_col if args.evidence_col in optional else None, args.owner_col if args.owner_col in optional else None, block_severities, block_statuses)
    if args.output_json:
