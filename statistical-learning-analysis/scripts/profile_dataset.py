...
def likely_id_column(column: dict[str, Any], row_count: int) -> bool:
    ...
    if name in {"id", "uuid", "guid"} or name.endswith("_id") or name.endswith("id"):
        return True
    return row_count >= 20 and unique_rate >= 0.98 and column["inferred_type"] not in {"numeric", "integer", "categorical_integer", "datetime"}


def build_profile(path: Path, target: str | None, group: str | None, time: str | None, max_rows: int) -> dict[str, Any]:
    ...
    if truncated:
        warnings.append(f"Profile uses first {sample_rows} rows out of {total_rows}; rerun with higher --max-rows if needed.")
    if sample_rows and len(header) > sample_rows and (len(header) > 100 or len(header) / sample_rows > 2):
        warnings.append("p > n in sampled data; prefer regularization, dimension reduction inside CV, and nested validation.")
    ...
    possible_time_cols = [c["name"] for c in columns if c["inferred_type"] == "datetime" or "date" in c["name"].lower() or "time" in c["name"].lower()]
    possible_group_cols = [
        c["name"]
        for c in columns
    ...
    if id_like_cols:
        warnings.append(f"ID-like columns: {', '.join(id_like_cols[:10])}; do not use as ordinary predictive features.")
    if possible_time_cols and not time:
        warnings.append(f"Possible time columns detected: {', '.join(possible_time_cols[:10])}; use time-aware splits if prediction time matters.")
    if possible_group_cols and not group:
        warnings.append(f"Possible group columns detected: {', '.join(possible_group_cols[:10])}; consider group-aware validation if rows repeat entities.")
    ...
    if target:
        if target not in values_by_col:
            ...
            if (target_profile.get("missing_rate") or 0) > 0:
                warnings.append(f"Target column '{target}' has missing values; define eligibility before modeling.")
            ...
            if target_type in {"boolean", "categorical", "categorical_integer"}:
                ...
                if total:
                    ...
                    if min_rate < 0.05:
                        warnings.append("Target appears imbalanced; avoid accuracy-only evaluation and tune thresholds on validation data.")
...
def suggest_next_steps(target_profile: dict[str, Any] | None, time_cols: list[str], group_cols: list[str], warnings: list[str]) -> list[str]:
    steps = []
    if target_profile is None:
        steps.append("No target was profiled; route to unsupervised learning, anomaly detection, or ask for the target variable.")
    else:
        target_type = target_profile["inferred_type"]
        if target_type in {"numeric", "integer"}:
            steps.append("For a continuous target, compare naive/linear baselines with regularized regression and tree ensembles.")
        elif target_type in {"boolean", "categorical", "categorical_integer"}:
            steps.append("For a categorical target, start with logistic/penalized logistic baselines and choose metrics before tuning.")
        elif target_type == "datetime":
            steps.append("Target appears time-like; verify whether this is survival/time-to-event or forecasting.")
        else:
...
def to_markdown(profile: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Columns", "", "| Column | Type | Missing | Unique | Examples |", "| --- | --- | --- | --- | --- |"])
...
def main() -> int:
    ...
    parser.add_argument("--max-rows", type=int, default=50000, help="Maximum rows to store/profile in detail.")
