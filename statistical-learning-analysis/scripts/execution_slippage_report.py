...
Standard-library only. Input is an order/fill CSV with side, quantity,
...
from quant_utils import mean, parse_float, quantile, read_dataframe, require_columns, sorted_group_keys, summarize_values
...
def summarize_orders(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    benchmark_slips = [item["benchmark_slippage_bps"] for item in items if item["benchmark_slippage_bps"] is not None]
    ...
    return {
        ...
        "notional_weighted_benchmark_slippage_bps": weighted_slippage([item for item in items if item["benchmark_slippage_bps"] is not None], "benchmark_slippage_bps"),
...
def build_report(
    ...
    for row in rows:
        ...
        if not date or not asset or side_sign is None or quantity is None or decision_price is None or fill_price is None or quantity == 0 or decision_price <= 0 or fill_price <= 0:
            dropped += 1
            continue
        ...
        benchmark_slippage_bps = side_sign * (fill_price - benchmark_price) / benchmark_price * 10000 if benchmark_price is not None else None
        participation = None
        if adv is not None:
    ...
    return {
        ...
        "notes": [
            ...
            "This report summarizes realized fills; it does not simulate queue position, partial fills, or market impact.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## By Side",
        "",
        "| Side | Orders | Notional | Cost dollars | Weighted slippage bps | Mean slippage bps | Mean participation |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for side, item in report["by_side"].items():
        lines.append(
            f"| {side} | {item['n_orders']} | {item['total_notional']} | {item['total_cost_dollars']} | {item['notional_weighted_slippage_bps']} | {item['mean_slippage_bps']} | {item['participation_summary']['mean']} |"
        )
    lines.extend(["", "## By Asset", "", "| Asset | Orders | Notional | Cost dollars | Weighted slippage bps | P90 slippage bps | Mean participation |", "| --- | --- | --- | --- | --- | --- | --- |"])
    for asset, item in report["by_asset"].items():
        lines.append(
            f"| {asset} | {item['n_orders']} | {item['total_notional']} | {item['total_cost_dollars']} | {item['notional_weighted_slippage_bps']} | {item['p90_slippage_bps']} | {item['participation_summary']['mean']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize execution slippage and implementation shortfall diagnostics.")
    ...
    require_columns(header, [args.date_col, args.asset_col, args.side_col, args.quantity_col, args.decision_price_col, args.fill_price_col] + optional)
