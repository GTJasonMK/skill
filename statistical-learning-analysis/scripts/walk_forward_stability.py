...
Standard-library only. Input should contain date, parameter identifier, and a
...
from quant_utils import mean, parse_float, read_dataframe, require_columns, sorted_group_keys, summarize_values
...
def aggregate_by_date_param(rows: list[dict[str, str]], date_col: str, param_col: str, metric_col: str) -> tuple[dict[str, dict[str, list[float]]], int]:
...
def window_param_mean(grouped: dict[str, dict[str, list[float]]], dates: list[str], params: list[str]) -> dict[str, float | None]:
...
def build_report(
    ...
    while start + train_periods + test_periods <= len(dates):
        ...
        if not valid_train:
            start += test_periods
            continue
        selected = max(valid_train, key=valid_train.get) if higher_is_better else min(valid_train, key=valid_train.get)
        ...
        if valid_test:
            ...
            regret = oracle - selected_test if higher_is_better and selected_test is not None else selected_test - oracle if selected_test is not None else None
        folds.append(
            {
                "fold": len(folds) + 1,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate walk-forward parameter-selection stability from long-form results.")
    ...
    report = build_report(rows, args.date_col, args.param_col, args.metric_col, args.train_periods, args.test_periods, not args.lower_is_better)
    if args.output_json:
