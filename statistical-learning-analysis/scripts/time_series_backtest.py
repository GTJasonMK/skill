...
def backtest(series: list[tuple[str, float]], horizon: int, seasonal_period: int, initial_window: int) -> dict[str, object]:
    y = [value for _, value in series]
    dates = [date for date, _ in series]
    errors: dict[str, dict[int, list[float]]] = {"naive": defaultdict(list), "seasonal_naive": defaultdict(list)}
    predictions = []
...
def markdown(result: dict[str, object]) -> str:
    ...
    for model, by_horizon in metrics.items():
        ...
        for horizon, values in by_horizon.items():
            ...
            lines.append(f"| {model} | {horizon} | {values['count']} | {values['mae']} | {values['rmse']} | {values['bias']} |")
    return "\n".join(lines) + "\n"


def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest naive and seasonal-naive forecasts for a CSV time series.")
    ...
    parser.add_argument("--initial-window", type=int, help="Minimum observations before first forecast origin.")
