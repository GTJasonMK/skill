...
def evaluate(y_true: list[str], scores: list[float], threshold: float, positive_label: str, negative_label: str) -> dict[str, float | None]:
    preds = [positive_label if score >= threshold else negative_label for score in scores]
    tp = sum(1 for t, p in zip(y_true, preds) if t == positive_label and p == positive_label)
    fp = sum(1 for t, p in zip(y_true, preds) if t != positive_label and p == positive_label)
    tn = sum(1 for t, p in zip(y_true, preds) if t != positive_label and p != positive_label)
    fn = sum(1 for t, p in zip(y_true, preds) if t == positive_label and p != positive_label)
    out = metrics(tp, fp, tn, fn)
    out["threshold"] = threshold
    return out


def choose_threshold(rows: list[dict[str, str]], truth: str, score: str, positive_label: str | None, objective: str, min_precision: float | None, min_recall: float | None, fp_cost: float, fn_cost: float) -> dict[str, object]:
    values = [(row.get(truth, ""), parse_float(row.get(score))) for row in rows if not is_missing(row.get(truth, ""))]
    ...
    if objective == "f1":
        key = lambda item: (item["f1"] is not None, item["f1"] or -1)
    elif objective == "precision":
        key = lambda item: (item["precision"] is not None, item["precision"] or -1)
    elif objective == "recall":
        key = lambda item: (item["recall"] is not None, item["recall"] or -1)
    elif objective == "specificity":
        key = lambda item: (item["specificity"] is not None, item["specificity"] or -1)
    elif objective == "cost":
...
def markdown(report: dict[str, object]) -> str:
    ...
    for item in report["top_results"]:
        lines.append(
            f"| {item['threshold']} | {item['precision']} | {item['recall']} | {item['specificity']} | {item['accuracy']} | {item['f1']} | {item['fp']} | {item['fn']} | {item['expected_cost']} |"
        )
    lines.extend(["", "The chosen threshold should be validated on held-out data, not reused as a final test statistic."])
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument("--objective", choices=["f1", "precision", "recall", "specificity", "cost"], default="f1")
    ...
    report = choose_threshold(rows, args.truth, args.score, args.positive_label, args.objective, args.min_precision, args.min_recall, args.fp_cost, args.fn_cost)
    if args.output_json:
