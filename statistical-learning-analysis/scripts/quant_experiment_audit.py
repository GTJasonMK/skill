...
Standard-library only. This script reads a CSV log of tested alpha factors,
...
from quant_utils import parse_float, read_dataframe, summarize_series as summarize_values
...
TRUE_VALUES = {"1", "true", "t", "yes", "y", "selected", "promoted", "approved", "pass", "passed", "winner", "live", "paper"}
FALSE_VALUES = {"0", "false", "f", "no", "n", "rejected", "reject", "fail", "failed", "dropped", "abandoned", "killed"}
SELECTED_STATUS = {"selected", "promoted", "approved", "pass", "passed", "winner", "paper", "paper_trading", "live"}
...
def finding(severity: str, check: str, detail: str, metric: Any = None, threshold: Any = None, ids: list[str] | None = None) -> dict[str, Any]:
...
def compact_family_summary(family: str, rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    final_tested = [row for row in rows if boolish(row, args.final_test_col) is True or has_value(row, args.test_metric_col)]
    return {
        "family": family,
...
def metric_degradation(rows: list[dict[str, str]], args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[float], int]:
...
def multiple_testing_summary(rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    if not args.p_col:
        return {"p_col": args.p_col, "tests_used": 0, "rows_dropped": 0, "discoveries": {}, "selected_raw_not_bh": []}
    ...
    selected_raw_not_bh = []
    for item, qvalue in zip(tests, adjusted):
...
def build_report(header: list[str], rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    missing_core = [name for name in ["family_col", "status_col", "predeclared_col", "final_test_col"] if not coverage.get(name)]
    if missing_core:
        findings.append(finding("warning", "missing_audit_columns", "Core audit columns are missing.", missing_core))
    ...
    predeclared_false = [experiment_id(row, args.id_col, i) for i, row in enumerate(rows, start=1) if boolish(row, args.predeclared_col) is False]
    predeclared_missing = [experiment_id(row, args.id_col, i) for i, row in enumerate(rows, start=1) if coverage.get("predeclared_col") and boolish(row, args.predeclared_col) is None]
    ...
    data_version_missing = [experiment_id(row, args.id_col, i) for i, row in enumerate(rows, start=1) if coverage.get("data_version_col") and not has_value(row, args.data_version_col)]
    code_version_missing = [experiment_id(row, args.id_col, i) for i, row in enumerate(rows, start=1) if coverage.get("code_version_col") and not has_value(row, args.code_version_col)]

    if failed_rows == [] and n >= args.min_registry_size_for_failure_warning:
        findings.append(finding("warning", "no_failed_trials_recorded", "No failed, rejected, dropped, or abandoned trials are recorded; the registry may omit unsuccessful experiments.", 0, ">=1"))
    if pct(len(predeclared_false) + len(predeclared_missing), n) is not None and pct(len(predeclared_false) + len(predeclared_missing), n) > args.max_unregistered_rate:
        findings.append(
            finding(
                "warning",
    ...
    if final_missing_selected and pct(len(final_missing_selected), max(len(selected_rows), 1)) > args.max_selected_missing_final_test_rate:
        findings.append(
            finding(
                "blocker",
    ...
    if coverage.get("data_version_col") and data_version_missing:
        findings.append(finding("warning", "missing_data_version", "Some experiments lack data-version evidence.", len(data_version_missing), 0, data_version_missing))
    if coverage.get("code_version_col") and code_version_missing:
        findings.append(finding("warning", "missing_code_version", "Some experiments lack code-version evidence.", len(code_version_missing), 0, code_version_missing))

    degradation_records, degradations, selected_metric_missing = metric_degradation(rows, args)
    selected_degraded = [item for item in degradation_records if item["selected"] and item["degradation"] > args.max_degradation]
    if selected_metric_missing:
        findings.append(finding("warning", "selected_missing_metric_pair", "Selected experiments are missing validation or final-test metric pairs.", selected_metric_missing, 0))
    ...
    mt = multiple_testing_summary(rows, args) if coverage.get("p_col") else {"p_col": args.p_col, "tests_used": 0, "rows_dropped": 0, "discoveries": {}, "selected_raw_not_bh": []}
    ...
    families = [compact_family_summary(family, family_rows, args) for family, family_rows in sorted(family_map.items())]
    ...
    return {
        ...
        "largest_validation_test_degradations": sorted(degradation_records, key=lambda item: item["degradation"], reverse=True)[:20],
        "multiple_testing": mt,
        "families": families,
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Family Summary", "", "| Family | Experiments | Selected | Failed | Predeclared | Final tested |", "| --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit a quant research experiment registry for data-snooping and selective-reporting risk.")
