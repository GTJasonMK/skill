...
def _selected_positions(items: pd.DataFrame, asset_col: str, factor_col: str,
                        top_frac: float, side: str) -> dict[str, float]:
...
def _rank_autocorrelation(prev_items: pd.DataFrame, cur_items: pd.DataFrame,
                          asset_col: str, factor_col: str) -> float | None:
...
def build_report(
    ...
    for prev, cur in zip(snapshots, snapshots[1:]):
        transitions.append({
            ...
            "rank_autocorrelation": _rank_autocorrelation(prev["items"], cur["items"], asset_col, factor_col),
        })
    ...
    rank_autos = [item["rank_autocorrelation"] for item in transitions if item["rank_autocorrelation"] is not None]
    return {
        "date_col": date_col,
        "asset_col": asset_col,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Estimate selected-name and factor-rank turnover across rebalance dates.")
    ...
    report = build_report(df, args.date_col, args.asset_col, args.factor_col,
                         args.top_frac, args.side, args.min_assets_per_date)
    if args.output_json:
