...
def build_report(
    ...
    return {
        ...
        "bin_table": bin_rows,
        "notes": [
            "ECE is the count-weighted mean absolute gap between predicted probability and observed positive rate.",
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["bin_table"]:
        ...
        lines.append(f"| {row['bin']} | {rng} | {row['count']} | {row['mean_score']} | {row['fraction_positive']} | {row['abs_gap']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probability calibration diagnostics (reliability + ECE + Brier).")
