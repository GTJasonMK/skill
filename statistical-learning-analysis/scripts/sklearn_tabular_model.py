...
def require_dependencies() -> dict[str, Any]:
    ...
    if missing:
        raise SystemExit("Missing dependencies: " + ", ".join(sorted(set(missing))) + ". Install them before running this script.")
    return locals()
...
def build_pipeline(task: str, model_name: str, deps: dict[str, Any], numeric_cols: list[str], categorical_cols: list[str]) -> Any:
    ...
    categorical = Pipeline([("imputer", SimpleImputer(strategy="most_frequent")), ("onehot", one_hot_encoder(OneHotEncoder))])
...
def split_data(df: Any, target: str, task: str, args: argparse.Namespace, deps: dict[str, Any]) -> tuple[Any, Any, Any, Any, dict[str, Any]]:
    ...
    if args.time_col:
        ...
        if task == "classification":
            ...
            if counts.min() >= 2:
                stratify = df[target]
        train, test = train_test_split(df, test_size=args.test_size, random_state=args.seed, stratify=stratify)
        sort_info["split"] = "random_stratified" if stratify is not None else "random"
    feature_cols = [c for c in df.columns if c != target and c not in {args.time_col, args.group_col} and c not in args.exclude]
    return train[feature_cols], test[feature_cols], train[target], test[target], sort_info


def classification_metrics(y_true: Any, y_pred: Any, proba: Any, deps: dict[str, Any]) -> dict[str, float | None]:
    metrics = {
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Columns", "", f"- Numeric: {', '.join(report['numeric_columns']) or 'None'}", f"- Categorical: {', '.join(report['categorical_columns']) or 'None'}"])
    lines.extend(["", "## Caveats", "", "- This is a starter pipeline. Review leakage, target timing, split design, calibration, and task-specific diagnostics before using results."])
    return "\n".join(lines) + "\n"


def main() -> int:
    parser = argparse.ArgumentParser(description="Train a starter scikit-learn tabular classifier/regressor from CSV.")
    ...
    (args.output_dir / "model_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
