...
Standard-library only. This script checks whether a date-entity signal panel
...
def audit_row(
    ...
    if not entity:
        ...
        add_issue(findings, row_issues, "blocker", "missing_entity", "Required entity identifier is missing.", row_number, entity, None)
    if as_of_dt is None:
        add_issue(findings, row_issues, "blocker", "invalid_as_of_date", "Decision/as-of date is missing or invalid.", row_number, entity, as_of_raw or None, as_of_raw or None)
        return {
    ...
    available_dt = check_optional_date(header, row, args.available_date_col, "available_date", row_number, entity, as_of_key, findings, row_issues, "blocker")
    data_dt = check_optional_date(header, row, args.data_date_col, "data_date", row_number, entity, as_of_key, findings, row_issues, "blocker")
    period_end_dt = check_optional_date(header, row, args.period_end_col, "period_end_date", row_number, entity, as_of_key, findings, row_issues, "warning")
    signal_dt = check_optional_date(header, row, args.signal_date_col, "signal_date", row_number, entity, as_of_key, findings, row_issues, "warning")
    rebalance_dt = check_optional_date(header, row, args.rebalance_date_col, "rebalance_date", row_number, entity, as_of_key, findings, row_issues, "warning")
    execution_dt = check_optional_date(header, row, args.execution_date_col, "execution_date", row_number, entity, as_of_key, findings, row_issues, "warning")
    universe_dt = check_optional_date(header, row, args.universe_date_col, "universe_date", row_number, entity, as_of_key, findings, row_issues, "warning")
    revision_dt = check_optional_date(header, row, args.revision_timestamp_col, "revision_timestamp", row_number, entity, as_of_key, findings, row_issues, "warning")
    vendor_dt = check_optional_date(header, row, args.vendor_timestamp_col, "vendor_timestamp", row_number, entity, as_of_key, findings, row_issues, "warning")
    ...
    if present(header, args.available_date_col) and available_dt is None:
        ...
        add_issue(findings, row_issues, severity, "missing_available_date", "Availability/release timestamp is missing.", row_number, entity, as_of_key)
    if not present(header, args.available_date_col):
        severity = "blocker" if args.strict_missing_availability else "warning"
        add_issue(findings, row_issues, severity, "missing_available_date_column", "No availability/release timestamp column is present.", row_number, entity, as_of_key)
    if present(header, args.data_date_col) and data_dt is None:
        add_issue(findings, row_issues, "warning", "missing_data_date", "Source data date is missing.", row_number, entity, as_of_key)

    compare_after(findings, row_issues, "available_date", available_dt, "as_of", as_of_dt, "available_date_after_as_of", "Data availability is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "data_date", data_dt, "as_of", as_of_dt, "data_date_after_as_of", "Source data date is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "period_end_date", period_end_dt, "as_of", as_of_dt, "period_end_after_as_of", "Reporting period end is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "signal_date", signal_dt, "as_of", as_of_dt, "signal_date_after_as_of", "Signal timestamp is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "available_date", available_dt, "signal_date", signal_dt, "available_date_after_signal_date", "Signal timestamp precedes the data availability timestamp.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "period_end_date", period_end_dt, "available_date", available_dt, "period_end_after_available_date", "Reporting period end is after availability timestamp.", row_number, entity, as_of_key, "warning")
    compare_after(findings, row_issues, "universe_date", universe_dt, "as_of", as_of_dt, "universe_date_after_as_of", "Universe eligibility timestamp is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "revision_timestamp", revision_dt, "as_of", as_of_dt, "revision_after_as_of", "Revision timestamp is after the decision date; the row may use a later vintage.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "vendor_timestamp", vendor_dt, "as_of", as_of_dt, "vendor_timestamp_after_as_of", "Vendor timestamp is after the decision date.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "signal_date", signal_dt, "rebalance_date", rebalance_dt, "signal_after_rebalance", "Signal timestamp is after the rebalance timestamp.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "rebalance_date", rebalance_dt, "execution_date", execution_dt, "rebalance_after_execution", "Rebalance timestamp is after the execution timestamp.", row_number, entity, as_of_key)
    compare_after(findings, row_issues, "signal_date", signal_dt, "execution_date", execution_dt, "signal_after_execution", "Signal timestamp is after the execution timestamp.", row_number, entity, as_of_key)
    ...
    if present(header, args.in_universe_col):
        ...
        if in_universe is True and not present(header, args.universe_date_col):
            add_issue(findings, row_issues, "warning", "missing_universe_date_column", "Rows marked in-universe have no universe timestamp column.", row_number, entity, as_of_key)
        elif in_universe is True and universe_dt is None:
            add_issue(findings, row_issues, "warning", "missing_universe_date", "Row is marked in-universe but has no universe timestamp.", row_number, entity, as_of_key)
...
def build_report(header: list[str], rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "problem_rows": sorted(problem_rows, key=lambda item: (-int(item["issue_count"]), str(item["entity"]), str(item["as_of"])))[: args.finding_limit],
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Top Findings", "", "| Severity | Check | Row | Entity | As of | Metric | Threshold | Detail |", "| --- | --- | --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit point-in-time data alignment and look-ahead leakage risks.")
    ...
    parser.add_argument("--available-date-col", default="available_date", help="Release, filing, or vendor availability timestamp column.")
    parser.add_argument("--data-date-col", default="data_date", help="Source data observation timestamp column.")
    parser.add_argument("--period-end-col", default="period_end_date", help="Reporting period end timestamp column.")
    parser.add_argument("--signal-date-col", default="signal_date", help="Signal timestamp column.")
    parser.add_argument("--rebalance-date-col", default="rebalance_date", help="Portfolio rebalance timestamp column.")
    parser.add_argument("--execution-date-col", default="execution_date", help="Execution timestamp column.")
    parser.add_argument("--universe-date-col", default="universe_date", help="Universe eligibility timestamp column.")
    parser.add_argument("--in-universe-col", default="in_universe", help="Optional boolean universe membership column.")
    parser.add_argument("--revision-timestamp-col", default="revision_timestamp", help="Data revision or vintage timestamp column.")
    parser.add_argument("--vendor-timestamp-col", default="vendor_timestamp", help="Vendor ingest or effective timestamp column.")
    ...
    parser.add_argument("--strict-missing-availability", action="store_true", help="Treat missing availability timestamps as blockers.")
