...
def build_report(
    ...
    if spread_bps_col:
        ...
        df["__spread_bps"] = 0.0
        spread_bps_col_eff = "__spread_bps"
    ...
    for date, g in df.groupby(date_col, sort=True):
        liquidity = {row[asset_col]: {"adv": float(row[adv_col]), "spread_bps": float(row[spread_eff])}
    ...
    return {
        ...
        "binding_capacity_summary": summarize_series([item["binding_nav_capacity"] for item in periods if item["binding_nav_capacity"] is not None]),
        "periods": periods,
        "notes": [
            ...
            "This does not model intraday volume curves, queue priority, borrow, halts, or nonlinear liquidity regimes.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["periods"]:
        lines.append(f"| {item['date']} | {item['one_way_turnover']} | {item['max_adv_participation']} | {item['estimated_cost_bps']} | {item['binding_nav_capacity']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Estimate simple capacity and market-impact diagnostics from weight changes.")
    ...
    parser.add_argument("--impact-bps", type=float, default=50.0, help="Impact bps at full ADV participation before power scaling.")
    ...
    require_columns(df, [args.date_col, args.asset_col, args.weight_col, args.adv_col]
                    + ([args.spread_bps_col] if args.spread_bps_col else []))
    report = build_report(df, args.date_col, args.asset_col, args.weight_col, args.adv_col,
                         args.spread_bps_col, args.nav, args.max_participation, args.impact_bps, args.impact_power)
    if args.output_json:
