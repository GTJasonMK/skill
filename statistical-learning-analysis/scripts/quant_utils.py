...
def summarize_series(s) -> dict[str, Any]:
    ...
    n = int(len(series))
    if n == 0:
        return {"n": 0, "mean": None, "stdev": None, "t_stat": None,
...
def summarize_returns(returns, annualization: int, rf_annual: float = 0.0) -> dict[str, Any]:
    ...
    if n == 0:
        out: dict[str, Any] = {
            "n": 0, "mean_return": None, "annualized_return_geometric": None,
            "annualized_return_arithmetic": None, "volatility": None,
            "annualized_volatility": None, "sharpe": None,
            "historical_var_95": None, "historical_expected_shortfall_95": None,
        }
...
def cross_sectional_corr(df: pd.DataFrame, group: str, x: str, y: str,
