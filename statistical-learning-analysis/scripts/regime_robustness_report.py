...
Standard-library only. Input is a time-series CSV with a strategy return column
...
from quant_utils import correlation, parse_float, read_dataframe, require_columns, sorted_group_keys, stdev, summarize_returns
...
def build_report(
    ...
    for regime in sorted_group_keys(list(by_regime)):
        ...
        corr = correlation(returns, benchmark_returns) if benchmark_col and len(benchmark_returns) == len(returns) else None
    ...
    worst_mean = min(valid_regimes, key=lambda item: item["mean_return"] if item["mean_return"] is not None else float("inf"), default=None)
    worst_drawdown = min(valid_regimes, key=lambda item: item["max_drawdown"] if item["max_drawdown"] is not None else 0.0, default=None)
    ...
    return {
        ...
        "overall_return_summary": summarize_returns([item["return"] for item in records], annualization, risk_free_annual),
...
def main() -> int:
    ...
    require_columns(header, [args.date_col, args.return_col, args.regime_col] + ([args.benchmark_col] if args.benchmark_col else []))
    report = build_report(rows, args.date_col, args.return_col, args.regime_col, args.benchmark_col, args.annualization, args.risk_free_annual, args.min_count)
    if args.output_json:
