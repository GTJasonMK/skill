...
def build_report(
    ...
    for date, g in df.groupby(date_col, sort=True):
        ...
        for asset in universe:
            ...
            if adv_col and nav is not None and meta is not None:
                ...
                if adv > 0:
                    ...
                    max_adv_participation = participation if max_adv_participation is None else max(max_adv_participation, participation)
        ...
        if return_col:
            ...
            gross_return = sum(w * float(returns_by_asset[a]) for a, w in cur_weights.items() if a in returns_by_asset)
    ...
    participation = [item["max_adv_participation"] for item in periods if item["max_adv_participation"] is not None]
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if report["max_adv_participation_summary"]:
        ...
        lines.extend(["", f"- Mean max ADV participation: {part['mean']}", f"- Max ADV participation: {part['max']}"])
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Estimate turnover-driven transaction costs from portfolio weights.")
    ...
    report = build_report(df, args.date_col, args.asset_col, args.weight_col, args.return_col,
                         args.spread_bps_col, args.adv_col, args.annualization, args.commission_bps,
                         args.slippage_bps, args.borrow_bps_annual, args.nav)
    if args.output_json:
