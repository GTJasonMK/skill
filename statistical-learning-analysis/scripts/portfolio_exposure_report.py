...
def build_report(
    ...
    exposure_summary = {
        ...
        "concentration_hhi": summarize_series([item["concentration_hhi"] for item in by_date if item["concentration_hhi"] is not None]),
    }
    return {
        "date_col": date_col,
        "asset_col": asset_col,
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["numeric_exposure_summary"]:
        lines.append(f"| {row['name']} | {row['n']} | {row['mean']} | {row['stdev']} | {row['min']} | {row['max']} |")
    lines.extend(["", "Notes:"])
