...
def finding(severity: str, check: str, detail: str, source: str, metric: Any = None, threshold: Any = None) -> dict[str, Any]:
...
def evaluate_report(data: Any, source: str, report_type: str, args: argparse.Namespace) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    if not isinstance(data, dict):
    ...
    if report_type == "point_in_time_audit":
        ...
        if not findings:
            ...
            if blockers > 0:
                findings.append(finding("blocker", "point_in_time_blockers", "Point-in-time audit has blocker findings.", source, blockers, 0))
            if warnings > 0:
                findings.append(finding("warning", "point_in_time_warnings", "Point-in-time audit has warning findings.", source, warnings, 0))
    elif report_type == "execution_timing_audit":
        top = data.get("top_findings")
        if isinstance(top, list):
            for item in top[:10]:
        ...
        if not findings:
            ...
            warnings = as_int(data.get("warning_count")) or 0
            if blockers > 0:
                findings.append(finding("blocker", "execution_timing_blockers", "Execution timing audit has blocker findings.", source, blockers, 0))
            if warnings > 0:
                findings.append(finding("warning", "execution_timing_warnings", "Execution timing audit has warning findings.", source, warnings, 0))
    elif report_type == "tradability_audit":
        top = data.get("top_findings")
        if isinstance(top, list):
            for item in top[:10]:
        ...
        if not findings:
            ...
            warnings = as_int(data.get("warning_count")) or 0
            if blockers > 0:
                findings.append(finding("blocker", "tradability_blockers", "Tradability audit has blocker findings.", source, blockers, 0))
            if warnings > 0:
                findings.append(finding("warning", "tradability_warnings", "Tradability audit has warning findings.", source, warnings, 0))
    elif report_type == "experiment_audit":
        top = data.get("top_findings")
        if isinstance(top, list):
            for item in top[:10]:
        ...
        if not findings:
            ...
            warnings = as_int(data.get("warning_count")) or 0
            if blockers > 0:
                findings.append(finding("blocker", "experiment_audit_blockers", "Experiment audit has blocker findings.", source, blockers, 0))
            if warnings > 0:
                findings.append(finding("warning", "experiment_audit_warnings", "Experiment audit has warning findings.", source, warnings, 0))
    elif report_type == "factor_ic":
        check_min(findings, as_float(get_path(data, ("periods_used",))), args.min_dates, "blocker", "factor_ic_sample", source, "Missing IC sample length.", "Too few IC periods for research gate.")
        check_min(findings, as_float(get_path(data, ("rank_ic_summary", "mean"))), args.min_rank_ic_mean, "blocker", "rank_ic_mean", source, "Missing rank IC mean.", "Rank IC mean is below threshold.")
        check_min(findings, as_float(get_path(data, ("rank_ic_summary", "positive_rate"))), args.min_rank_ic_positive_rate, "warning", "rank_ic_positive_rate", source, "Missing rank IC positive rate.", "Rank IC positive rate is weak.")
        check_min(findings, as_float(get_path(data, ("rank_ic_summary", "t_stat"))), args.min_rank_ic_t_stat, "warning", "rank_ic_t_stat", source, "Missing rank IC t-stat.", "Rank IC t-stat is weak.")
    elif report_type == "incremental_alpha":
        assessment = str(data.get("assessment", ""))
        check_min(findings, as_float(get_path(data, ("dates_used",))), args.min_dates, "blocker", "incremental_sample", source, "Missing incremental-alpha sample length.", "Too few incremental-alpha dates for research gate.")
        check_min(findings, as_float(get_path(data, ("residual_rank_ic_summary", "mean"))), args.min_residual_rank_ic_mean, "blocker", "residual_rank_ic_mean", source, "Missing residual rank IC mean.", "Residual rank IC mean is below threshold.")
        check_min(findings, as_float(get_path(data, ("residual_rank_ic_summary", "positive_rate"))), args.min_residual_positive_rate, "warning", "residual_rank_ic_positive_rate", source, "Missing residual rank IC positive rate.", "Residual rank IC positive rate is weak.")
        if assessment in {"weak_or_negative_incremental_evidence", "insufficient_signal_variation", "insufficient_dates"}:
            findings.append(finding("blocker", "incremental_assessment", "Incremental alpha assessment does not support promotion.", source, assessment))
        check_max(findings, as_float(get_path(data, ("candidate_explained_by_base_r2_summary", "mean"))), args.max_candidate_base_r2, "warning", "candidate_spanned_by_base", source, "Missing candidate-spanned-by-base R-squared.", "Candidate is highly explained by existing signals or exposures.")
    elif report_type == "signal_overlap":
        redundant = as_int(data.get("redundant_pair_count"))
        if redundant is None:
            findings.append(finding("warning", "signal_overlap_redundancy", "Missing redundant pair count.", source))
        elif redundant > args.max_redundant_pairs:
            severity = "blocker" if args.fail_on_redundant_pairs else "warning"
            findings.append(finding(severity, "signal_overlap_redundancy", "Too many redundant signal pairs.", source, redundant, args.max_redundant_pairs))
    elif report_type == "turnover":
        check_max(findings, as_float(get_path(data, ("turnover_summary", "mean"))), args.max_turnover, "warning", "turnover_mean", source, "Missing turnover mean.", "Turnover is above research gate threshold.")
    elif report_type == "transaction_cost":
        check_max(findings, as_float(data.get("mean_cost_bps_per_period")), args.max_cost_bps, "blocker", "cost_bps", source, "Missing mean cost bps per period.", "Mean implementation cost is above threshold.")
        check_max(findings, as_float(get_path(data, ("max_adv_participation_summary", "max"))), args.max_adv_participation, "warning", "adv_participation", source, "Missing ADV participation; capacity evidence may be incomplete.", "ADV participation is above threshold.")
    elif report_type == "capacity":
        check_max(findings, as_float(get_path(data, ("participation_summary", "max"))), args.max_adv_participation, "blocker", "adv_participation", source, "Missing max ADV participation.", "ADV participation is above threshold.")
        check_max(findings, as_float(get_path(data, ("cost_bps_summary", "mean"))), args.max_cost_bps, "blocker", "capacity_cost_bps", source, "Missing capacity cost bps.", "Estimated capacity cost is above threshold.")
        min_capacity = as_float(get_path(data, ("binding_capacity_summary", "min")))
        if args.min_binding_capacity is not None:
            check_min(findings, min_capacity, args.min_binding_capacity, "blocker", "binding_capacity", source, "Missing binding capacity estimate.", "Binding NAV capacity is below required minimum.")
    elif report_type == "multiple_testing":
        raw = as_int(get_path(data, ("discoveries", "raw"))) or 0
        bh = as_int(get_path(data, ("discoveries", "bh_fdr"))) or 0
        if raw > 0 and bh <= 0:
            findings.append(finding("blocker", "multiple_testing", "Raw discoveries do not survive Benjamini-Hochberg FDR.", source, {"raw": raw, "bh_fdr": bh}))
        if as_int(data.get("tests_used")) is None:
            findings.append(finding("warning", "multiple_testing_scope", "Missing tested family size.", source))
    elif report_type == "bootstrap_reality_check":
        ...
        check_max(findings, p_best, args.max_reality_check_p_value, "blocker", "reality_check_p_value", source, "Missing reality-check p-value.", "Reality-check p-value is too high after strategy search.")
    elif report_type == "long_short_backtest":
        check_min(findings, as_float(get_path(data, ("periods_used",))), args.min_dates, "blocker", "backtest_sample", source, "Missing backtest sample length.", "Too few backtest periods for research gate.")
        check_min(findings, as_float(get_path(data, ("net_return_summary", "sharpe"))), args.min_net_sharpe, "warning", "net_sharpe", source, "Missing net Sharpe.", "Net Sharpe is below threshold.")
        check_min(findings, as_float(get_path(data, ("net_return_summary", "annualized_return_geometric"))), args.min_net_annual_return, "warning", "net_return", source, "Missing net annualized return.", "Net annualized return is below threshold.")
        check_max(findings, as_float(get_path(data, ("turnover_summary", "mean"))), args.max_turnover, "warning", "backtest_turnover", source, "Missing backtest turnover.", "Backtest turnover is above threshold.")
    return findings
...
def build_report(paths: list[Path], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for item in missing:
        ...
        all_findings.append(finding(severity, "missing_required_diagnostic", f"Required diagnostic type is missing: {item}.", "gate", item))
    ...
    ranked_findings = sorted(all_findings, key=lambda item: (severity_rank(item["severity"]), item["check"], item["source"]), reverse=True)
    ...
    return {
        ...
        "notes": [
            ...
            "Required diagnostic types and the experiment registry should be predeclared before reviewing results; missing diagnostics are evidence gaps.",
            "Passing this gate does not remove the need for point-in-time data review, implementation-cost calibration, risk-owner approval, and out-of-sample monitoring.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["reports"]:
        ...
        lines.append(f"| {md_escape(item['name'])} | {md_escape(item['type'])} | {md_escape(metrics)} | {item['finding_count']} |")

    lines.extend(["", "## Findings", "", "| Severity | Check | Source | Metric | Threshold | Detail |", "| --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gate candidate alpha research using bundled quant JSON diagnostics.")
    parser.add_argument("json_paths", nargs="+", type=Path)
    parser.add_argument("--required-types", default=",".join(DEFAULT_REQUIRED_TYPES), help="Comma-separated required diagnostic type aliases.")
    parser.add_argument("--strict-missing", action="store_true", help="Treat missing required diagnostics as blockers instead of warnings.")
    ...
    for name in ["min_rank_ic_positive_rate", "min_residual_positive_rate", "max_candidate_base_r2", "max_adv_participation", "max_reality_check_p_value"]:
