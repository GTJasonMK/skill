...
def date_regression(g: pd.DataFrame, return_col: str, feature_cols: list[str],
                    intercept: bool) -> tuple[dict[str, Any] | None, int]:
...
def build_report(
    ...
    for date, g in df.groupby(date_col, sort=True):
        ...
        if fit is None or fit["n"] < min_assets:
            skipped_dates += 1
            continue
        coeffs = {name: coef for name, coef in zip(names, fit["coefficients"])}
        by_date.append({
    ...
    for name in names:
        ...
        if name == "intercept":
            row["annualized_mean_arithmetic"] = summary["mean"] * annualization if summary["mean"] is not None else None
    ...
    return {
        ...
        "notes": [
            ...
            "For overlapping returns, autocorrelated premia, or clustered assets, use HAC/clustered inference in a full implementation.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["coefficient_summary"]:
        lines.append(
            f"| {row['name']} | {row['n']} | {row['mean']} | {row['stdev']} | {row['t_stat']} | {row['positive_rate']} | {row.get('annualized_mean_arithmetic', '')} |"
        )
...
def main() -> int:
    ...
    parser.add_argument("--feature-cols", required=True, help="Comma-separated characteristic/exposure columns.")
    ...
    report = build_report(df, args.date_col, args.return_col, feature_cols,
                         args.min_assets_per_date, not args.no_intercept, args.annualization)
    if args.output_json:
