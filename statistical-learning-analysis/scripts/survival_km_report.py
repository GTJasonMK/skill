...
def _logrank(df: pd.DataFrame, group_col: str) -> dict[str, Any]:
    ...
    for t in times:
        ...
        d_g = np.array([float(((df[group_col] == g) & (df["t"] == t) & (df["e"] == 1)).sum()) for g in groups])
        observed += d_g
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for name, info in report["by_group"].items():
        lines.append(f"| {name} | {info['n_subjects']} | {info['n_events']} | {info['n_censored']} | {info['median_survival']} |")
    if report["log_rank"]:
        lr = report["log_rank"]
        lines.extend(["", "## Log-Rank Test", "",
