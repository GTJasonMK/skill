...
Standard-library only. Input is a long CSV with date, asset, signal value, and
...
from quant_utils import correlation, mean, parse_float, read_dataframe, require_columns, sorted_group_keys, spearman, stdev, summarize_values
...
def build_report(
    ...
    for row in rows:
        ...
        if not date or not asset or signal is None or forward_return is None:
            dropped += 1
            continue
        grouped.setdefault(date, []).append({"asset": asset, "signal": signal, "forward_return": forward_return})
    ...
    recent_turnover = [item["selection_turnover"] for item in recent if item["selection_turnover"] is not None]
    recent_n_assets = [item["n_assets"] for item in recent]
    recent_spreads = [item["top_minus_bottom_return"] for item in recent if item["top_minus_bottom_return"] is not None]
    ...
    if recent_n_assets and min(recent_n_assets) < min_assets:
        alerts.append({"severity": "warning", "name": "low_coverage", "detail": f"Recent minimum asset count below {min_assets}."})
    if recent_rank_ic_summary["mean"] is not None and recent_rank_ic_summary["mean"] < min_recent_rank_ic:
        alerts.append({"severity": "warning", "name": "weak_recent_rank_ic", "detail": f"Recent mean rank IC below {min_recent_rank_ic}."})
    if recent_rank_ic_summary["positive_rate"] is not None and recent_rank_ic_summary["positive_rate"] < min_positive_rate:
        alerts.append({"severity": "warning", "name": "low_rank_ic_positive_rate", "detail": f"Recent rank IC positive rate below {min_positive_rate}."})
    if recent_turnover_summary["mean"] is not None and recent_turnover_summary["mean"] > max_mean_turnover:
        alerts.append({"severity": "warning", "name": "high_selection_turnover", "detail": f"Recent mean selection turnover above {max_mean_turnover}."})
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if report["alerts"]:
        ...
        lines.append("- none")
    lines.extend(["", "## Per-Date Diagnostics", "", "| Date | Assets | IC | Rank IC | Top-bottom return | Turnover | Rank autocorr |", "| --- | --- | --- | --- | --- | --- | --- |"])
    for item in report["per_date"]:
        lines.append(f"| {item['date']} | {item['n_assets']} | {item['ic']} | {item['rank_ic']} | {item['top_minus_bottom_return']} | {item['selection_turnover']} | {item['rank_autocorrelation']} |")
    lines.extend(["", "Notes:"])
