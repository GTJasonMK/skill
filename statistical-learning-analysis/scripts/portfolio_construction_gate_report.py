...
def max_abs_category_exposure(latest: Any) -> float | None:
    ...
    for by_label in exposures.values():
        if isinstance(by_label, dict):
            values.extend(abs(value) for value in (as_float(v) for v in by_label.values()) if value is not None)
...
def evaluate_report(data: Any, source: str, report_type: str, args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    if report_type == "execution_timing_audit":
        ...
        check_min(findings, as_float(data.get("periods_used")), args.min_periods, "blocker", "backtest_sample", source, "Missing backtest sample length.", "Too few portfolio backtest periods.")
        ...
        if drawdown is None:
            findings.append(finding("warning", "net_drawdown", "Missing net max drawdown.", source, None, args.max_drawdown_abs))
        elif abs(drawdown) > args.max_drawdown_abs:
            findings.append(finding("warning", "net_drawdown", "Net max drawdown exceeds threshold.", source, drawdown, args.max_drawdown_abs))
        check_max(findings, as_float(get_path(data, ("turnover_summary", "mean"))), args.max_turnover, "warning", "backtest_turnover", source, "Missing turnover mean.", "Backtest turnover is above threshold.")
        check_max(findings, as_float(get_path(data, ("gross_exposure_summary", "max"))), args.max_gross_exposure, "blocker", "gross_exposure", source, "Missing gross exposure summary.", "Gross exposure exceeds threshold.")
    elif report_type == "portfolio_constraints":
        failed = as_int(data.get("periods_failed"))
        if failed is None:
            findings.append(finding("warning", "constraint_failures", "Missing constraint failure count.", source, None, args.max_constraint_failures))
        elif failed > args.max_constraint_failures:
            findings.append(finding("blocker", "constraint_failures", "Portfolio constraint violations exceed allowed count.", source, failed, args.max_constraint_failures))
        dropped = as_int(data.get("rows_dropped"))
        if dropped and dropped > 0:
            findings.append(finding("warning", "constraint_rows_dropped", "Constraint report dropped rows.", source, dropped))
    elif report_type == "portfolio_exposure":
        check_min(findings, as_float(data.get("periods_used")), args.min_periods, "warning", "exposure_sample", source, "Missing exposure sample length.", "Too few exposure periods.")
        check_max(findings, as_float(get_path(data, ("exposure_summary", "gross_exposure", "max"))), args.max_gross_exposure, "blocker", "exposure_gross", source, "Missing gross exposure summary.", "Gross exposure exceeds threshold.")
        check_abs_max(findings, max_abs_summary(get_path(data, ("exposure_summary", "net_exposure"))), args.max_abs_net_exposure, "blocker", "net_exposure", source, "Missing net exposure summary.", "Net exposure exceeds threshold.")
        check_max(findings, as_float(get_path(data, ("exposure_summary", "concentration_hhi", "max"))), args.max_concentration_hhi, "warning", "concentration_hhi", source, "Missing concentration HHI.", "Concentration HHI exceeds threshold.")
            ...
            for item in data.get("numeric_exposure_summary", []):
                if isinstance(item, dict):
                    ...
                    if value is not None and value > args.max_numeric_exposure_abs:
                        findings.append(finding("warning", "numeric_exposure", f"Numeric exposure {item.get('name')} exceeds absolute threshold.", source, value, args.max_numeric_exposure_abs))
        if args.max_category_abs_exposure is not None:
            ...
            if value is not None and value > args.max_category_abs_exposure:
                findings.append(finding("warning", "category_exposure", "Latest category exposure exceeds absolute threshold.", source, value, args.max_category_abs_exposure))
    elif report_type == "risk_contribution":
        check_max(findings, as_float(data.get("annualized_portfolio_volatility")), args.max_annualized_volatility, "warning", "portfolio_volatility", source, "Missing annualized portfolio volatility.", "Annualized portfolio volatility exceeds threshold.")
        check_max(findings, top_risk_share(data), args.max_single_asset_risk_share, "blocker", "risk_concentration", source, "Missing risk contribution concentration.", "Single asset risk contribution exceeds threshold.")
    elif report_type == "optimizer_sensitivity":
        check_max(findings, as_float(get_path(data, ("l1_distance_summary", "mean"))), args.max_optimizer_l1_distance, "warning", "optimizer_l1_distance", source, "Missing optimizer L1 distance.", "Optimized weights are too sensitive to input perturbations.")
        check_max(findings, max_sign_flip_rate(data), args.max_sign_flip_rate, "warning", "optimizer_sign_flip", source, "Missing optimizer sign flip rate.", "Optimized weights have high sign-flip sensitivity.")
        check_max(findings, as_float(get_path(data, ("concentration_hhi_summary", "mean"))), args.max_concentration_hhi, "warning", "optimizer_concentration", source, "Missing optimizer concentration.", "Optimizer concentration exceeds threshold.")
    elif report_type == "transaction_cost":
        check_max(findings, as_float(data.get("mean_cost_bps_per_period")), args.max_cost_bps, "blocker", "cost_bps", source, "Missing mean cost bps per period.", "Mean implementation cost exceeds threshold.")
        check_max(findings, as_float(get_path(data, ("max_adv_participation_summary", "max"))), args.max_adv_participation, "warning", "adv_participation", source, "Missing ADV participation; capacity evidence may be incomplete.", "ADV participation exceeds threshold.")
    elif report_type == "capacity":
        check_max(findings, as_float(get_path(data, ("participation_summary", "max"))), args.max_adv_participation, "blocker", "adv_participation", source, "Missing max ADV participation.", "ADV participation exceeds threshold.")
        check_max(findings, as_float(get_path(data, ("cost_bps_summary", "mean"))), args.max_cost_bps, "blocker", "capacity_cost_bps", source, "Missing capacity cost bps.", "Estimated capacity cost exceeds threshold.")
        if args.min_binding_capacity is not None:
            check_min(findings, as_float(get_path(data, ("binding_capacity_summary", "min"))), args.min_binding_capacity, "blocker", "binding_capacity", source, "Missing binding capacity estimate.", "Binding NAV capacity is below required minimum.")
    elif report_type == "alpha_research_gate":
        decision = str(data.get("gate_decision", "")).lower()
        if decision == "fail":
            findings.append(finding("blocker", "alpha_research_gate", "Alpha research gate failed before portfolio construction.", source, decision))
        elif decision == "review":
            findings.append(finding("warning", "alpha_research_gate", "Alpha research gate still requires review before portfolio construction.", source, decision))
    return findings
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gate portfolio construction using bundled quant JSON diagnostics.")
    ...
    if args.max_cost_bps < 0 or args.max_optimizer_l1_distance < 0 or args.max_annualized_volatility < 0:
        raise SystemExit("--max-cost-bps, --max-optimizer-l1-distance, and --max-annualized-volatility must be non-negative.")
