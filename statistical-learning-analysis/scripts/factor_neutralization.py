...
def _design_matrix(sub: pd.DataFrame, numeric_cols: list[str],
                   category_cols: list[str]) -> tuple[np.ndarray, dict[str, list[str]]]:
...
def neutralize_group(g: pd.DataFrame, signal_col: str, numeric_cols: list[str],
                     category_cols: list[str]) -> tuple[pd.DataFrame | None, dict[str, Any]]:
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["by_date"]:
        lines.append(f"| {item['date']} | {item['status']} | {item.get('n_raw')} | {item.get('n')} | {item.get('r2')} | {item.get('residual_std')} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Neutralize a factor signal against exposures within each date.")
    ...
    report, output_df = build_report(df, args.date_col, args.asset_col, args.signal_col,
                                     numeric_cols, category_cols, args.min_assets_per_date)
    if args.output_csv:
