...
def build_report(
    ...
    report = {
        ...
        "top_rows": top_rows,
        "notes": [
            "Z-score and IQR methods produce per-column scores aggregated by max; tune --threshold per scale.",
            "Mahalanobis assumes approximate multivariate normality; covariance is shrunk toward the diagonal to handle near-singularity.",
    ...
    if method == "mahalanobis":
        ...
        report["notes"].append(
            f"For Mahalanobis with {dof} features, threshold {threshold} corresponds to chi2 p ~ {report['chi2_p_at_threshold']:.4g}."
        )
