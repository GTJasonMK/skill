...
def summarize(rows: list[dict[str, str]], header: list[str], row_threshold: float, col_threshold: float) -> dict[str, object]:
    n = len(rows)
    column_stats = []
    row_missing_counts = []
    for row in rows:
    ...
    return {
        ...
        "rows_with_high_missing": sum(count / len(header) >= row_threshold for count in row_missing_counts) if header else 0,
...
def markdown(report: dict[str, object]) -> str:
    ...
    for item in report["column_stats"]:
        lines.append(f"| {item['column']} | {item['missing_count']} | {item['missing_rate']} | {item['flag_row_threshold']} |")
...
def main() -> int:
    ...
    parser.add_argument("--row-threshold", type=float, default=0.5, help="Flag rows with missing rate above this threshold.")
    parser.add_argument("--col-threshold", type=float, default=0.4, help="Flag columns with missing rate above this threshold.")
