...
def _quantile_returns(g: pd.DataFrame, factor_col: str, return_col: str,
                      weight_col: str | None, quantiles: int) -> dict[int, float | None]:
...
def build_report(
    ...
    for q in range(1, quantiles + 1):
        ...
        summary["quantile"] = q
        summary["annualized_return_arithmetic"] = summary["mean"] * annualization if summary["mean"] is not None else None
    ...
    spread_summary["annualized_return_arithmetic"] = spread_summary["mean"] * annualization if spread_summary["mean"] is not None else None
    return {
        "date_col": date_col,
...
def main() -> int:
    ...
    require_columns(df, [args.date_col, args.factor_col, args.forward_return_col] + ([args.weight_col] if args.weight_col else []))
    report = build_report(
        df, args.date_col, args.factor_col, args.forward_return_col,
        args.quantiles, args.min_assets_per_date, args.annualization, args.weight_col,
    )
    if args.output_json:
