...
def _base_design(sub: pd.DataFrame, numeric_base_cols: list[str],
                 category_base_cols: list[str], intercept: bool) -> tuple[np.ndarray, int]:
...
def date_incremental_diagnostic(
    ...
    p_full = p_base + 1
    base_summary = {"date": date, "n_raw": int(len(g)), "n": int(n), "dropped": int(dropped),
                    "p_base": int(p_base), "p_full": int(p_full)}
    if n < min_assets:
    ...
    delta_adj_r2 = (full_adj_r2 - base_fit["adj_r2"]) if full_adj_r2 is not None and base_fit["adj_r2"] is not None else None
    return {
        **base_summary,
        "status": "ok",
...
def assessment(dates_used: int, min_dates: int, residual_rank_summary: dict[str, Any],
               delta_r2_summary: dict[str, Any], min_residual_rank_ic: float,
               min_positive_rate: float) -> str:
    ...
    if residual_mean >= min_residual_rank_ic and residual_positive >= min_positive_rate and (delta_mean is None or delta_mean >= 0):
...
def build_report(
    ...
    for date, g in df.groupby(date_col, sort=True):
        ...
        by_date.append(date_incremental_diagnostic(
            date_str, g, return_col, candidate_col, numeric_base_cols,
            category_base_cols, min_assets, intercept,
        ))
    ...
    residual_rank_values = [item["residual_rank_ic"] for item in ok_dates if item["residual_rank_ic"] is not None]
    ...
    candidate_base_r2_values = [item["candidate_explained_by_base_r2"] for item in ok_dates if item["candidate_explained_by_base_r2"] is not None]
    ...
    return {
        ...
        "assessment": assessment(len(ok_dates), min_dates, residual_rank_summary, delta_r2_summary, min_residual_rank_ic, min_positive_rate),
        "by_date": by_date,
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## By Date", "", "| Date | Status | N | Raw rank IC | Residual rank IC | Delta R2 | Candidate coef | Candidate base R2 |", "| --- | --- | --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diagnose incremental value of a candidate alpha signal after controlling for existing signals or exposures.")
    ...
    require_columns(df, [args.date_col, args.asset_col, args.forward_return_col, args.candidate_col]
                    + numeric_base_cols + category_base_cols)
    report = build_report(
        df, args.date_col, args.asset_col, args.forward_return_col, args.candidate_col,
        numeric_base_cols, category_base_cols, args.min_assets_per_date, args.min_dates,
        not args.no_intercept, args.min_residual_rank_ic, args.min_positive_rate,
    )
    if args.output_json:
