...
def smd_numeric(treated_vals: list[float], control_vals: list[float]) -> tuple[float | None, float | None, float | None]:
...
def balance_rows(rows: list[dict[str, str]], treatment: str, covariates: list[str], treated_value: str, control_value: str) -> list[dict[str, object]]:
    ...
    for covariate in covariates:
        ...
        if is_numeric(values):
            treated_vals = [v for v in (parse_float(row.get(covariate, "")) for row in treated) if v is not None]
            control_vals = [v for v in (parse_float(row.get(covariate, "")) for row in control) if v is not None]
...
def write_rows(path: Path, rows: list[dict[str, object]]) -> None:
    fieldnames = ["covariate", "level", "type", "treated_mean", "control_mean", "smd", "abs_smd", "flag_abs_smd_gt_0_1"]
...
def markdown(rows: list[dict[str, object]], limit: int = 30) -> str:
    ...
    for row in sorted_rows[:limit]:
        ...
        lines.append(
            f"| {row['covariate']} | {row['level']} | {row['type']} | {row['treated_mean']} | {row['control_mean']} | {smd} | {row['flag_abs_smd_gt_0_1']} |"
        )
    flagged = sum(bool(row["flag_abs_smd_gt_0_1"]) for row in rows)
    lines.extend(["", f"Flagged rows with |SMD| > 0.1: {flagged}", "This is a balance diagnostic, not proof of causal identification."])
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument("--treated-value", help="Value representing treated group. Defaults to the second sorted value.")
    parser.add_argument("--control-value", help="Value representing control group. Defaults to the first sorted value.")
    parser.add_argument("--covariates", help="Comma-separated covariates. Defaults to all columns except treatment.")
    ...
    observed = sorted({row.get(args.treatment, "") for row in rows if not is_missing(row.get(args.treatment, ""))})
    if len(observed) != 2 and (args.treated_value is None or args.control_value is None):
        raise SystemExit("Treatment must have exactly two observed values unless both --treated-value and --control-value are provided.")
