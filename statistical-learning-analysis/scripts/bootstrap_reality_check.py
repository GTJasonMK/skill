...
Standard-library only. This is a first-pass data-snooping diagnostic: compare
...
def build_report(
    ...
    t_stats = [m / (vol / (n ** 0.5)) if vol not in {None, 0} else None for m, vol in zip(means, vols)]
    ...
    for _ in range(bootstrap_samples):
        ...
        sample_t = [
            ...
            for sample_mean, sample_vol in zip(sample_means, sample_vols)
        ]
    ...
    p_mean = sum(value >= observed_best_mean for value in boot_best_means) / len(boot_best_means) if boot_best_means else None
    p_t = sum(value >= observed_best_t for value in boot_best_tstats) / len(boot_best_tstats) if boot_best_tstats and observed_best_t is not None else None
    strategies = []
    for col, avg, vol, t_stat in zip(columns, means, vols, t_stats):
        strategies.append(
            {
                "strategy": col,
                "mean_return": avg,
    ...
    return {
        ...
        "notes": [
            "This is a simple centered block-bootstrap reality check, not a full White Reality Check implementation.",
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["strategies"]:
        lines.append(f"| {item['strategy']} | {item['mean_return']} | {item['annualized_return_arithmetic']} | {item['volatility']} | {item['t_stat_mean']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a simple block-bootstrap reality check across strategy return columns.")
    ...
    report = build_report(rows, columns, args.block_length, args.bootstrap_samples, args.seed, args.annualization)
    if args.output_json:
