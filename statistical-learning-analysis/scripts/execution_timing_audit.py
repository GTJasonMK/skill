...
Standard-library only. This script checks whether signal timestamps, rebalance
...
def get_timestamp(
    ...
    if parsed is None:
        add_issue(findings, row_issues, invalid_severity, f"invalid_{label}", f"{label} is not parseable as an ISO-style date or timestamp.", row_number, entity, signal_key, value)
...
def first_available_timestamp(
    ...
    for label, col in candidates:
        if present(header, col) and text(row, col):
            parsed, date_only = get_timestamp(header, row, col, label, row_number, entity, None, findings, row_issues, "blocker")
...
def compare_after(
    ...
    if left_value is not None and right_value is not None and left_value > right_value:
        add_issue(
            ...
            entity,
            signal_key,
            {"left": left_name, "left_value": iso(left_value), "right": right_name, "right_value": iso(right_value)},
        )
...
def check_weekend(
    ...
    if value is not None and value.weekday() >= 5:
        add_issue(findings, row_issues, severity, f"{label}_on_weekend", f"{label} falls on a weekend; confirm the market calendar and timestamp convention.", row_number, entity, signal_key, iso(value))
...
def audit_row(
    ...
    signal_label, signal_dt, signal_date_only = first_available_timestamp(
        header,
        row,
        [("signal_date", args.signal_date_col), ("decision_date", args.decision_date_col), ("date", args.date_col)],
        row_number,
        entity,
        findings,
        row_issues,
    ...
    if signal_dt is None:
        add_issue(findings, row_issues, "blocker", "missing_signal_or_decision_date", "No parseable signal, decision, or generic date timestamp is available.", row_number, entity, None)

    rebalance_dt, rebalance_date_only = get_timestamp(header, row, args.rebalance_date_col, "rebalance_date", row_number, entity, signal_key, findings, row_issues)
    execution_dt, execution_date_only = get_timestamp(header, row, args.execution_date_col, "execution_date", row_number, entity, signal_key, findings, row_issues)
    return_start_dt, return_start_date_only = get_timestamp(header, row, args.return_start_col, "return_start_date", row_number, entity, signal_key, findings, row_issues)
    return_end_dt, return_end_date_only = get_timestamp(header, row, args.return_end_col, "return_end_date", row_number, entity, signal_key, findings, row_issues)
    signal_execution_same_price = same_price(text(row, args.signal_price_col), text(row, args.execution_price_col)) if present(header, args.signal_price_col) and present(header, args.execution_price_col) else None
    execution_return_start_same_price = same_price(text(row, args.execution_price_col), text(row, args.return_start_price_col)) if present(header, args.execution_price_col) and present(header, args.return_start_price_col) else None

    if not present(header, args.execution_date_col):
        severity = "blocker" if args.strict_missing_execution else "warning"
        add_issue(findings, row_issues, severity, "missing_execution_date_column", "No execution timestamp column is present.", row_number, entity, signal_key)
    elif execution_dt is None:
        severity = "blocker" if args.strict_missing_execution else "warning"
        add_issue(findings, row_issues, severity, "missing_execution_date", "Execution timestamp is missing.", row_number, entity, signal_key)
    for col, dt, check, detail in [
        (args.return_start_col, return_start_dt, "missing_return_start_date", "Forward-return start timestamp is missing."),
        (args.return_end_col, return_end_dt, "missing_return_end_date", "Forward-return end timestamp is missing."),
    ]:
        if not present(header, col) or dt is None:
            add_issue(findings, row_issues, "warning", check if present(header, col) else f"{check}_column", detail, row_number, entity, signal_key)

    compare_after(findings, row_issues, "signal_date", signal_dt, "rebalance_date", rebalance_dt, "signal_after_rebalance", "Signal timestamp is after rebalance timestamp.", row_number, entity, signal_key)
    compare_after(findings, row_issues, "signal_date", signal_dt, "execution_date", execution_dt, "signal_after_execution", "Signal timestamp is after execution timestamp.", row_number, entity, signal_key)
    compare_after(findings, row_issues, "rebalance_date", rebalance_dt, "execution_date", execution_dt, "rebalance_after_execution", "Rebalance timestamp is after execution timestamp.", row_number, entity, signal_key)
    compare_after(findings, row_issues, "return_start_date", return_start_dt, "return_end_date", return_end_dt, "return_start_after_return_end", "Forward-return start is after return end.", row_number, entity, signal_key)

    if return_start_dt is not None and return_end_dt is not None and return_start_dt == return_end_dt:
        add_issue(findings, row_issues, "blocker", "nonpositive_return_window", "Forward-return start and end timestamps are identical.", row_number, entity, signal_key, iso(return_start_dt))
    if signal_dt is not None and return_start_dt is not None and return_start_dt < signal_dt:
        add_issue(findings, row_issues, "blocker", "return_starts_before_signal", "Forward-return window starts before the signal timestamp.", row_number, entity, signal_key, {"signal_date": iso(signal_dt), "return_start_date": iso(return_start_dt)})
    if execution_dt is not None and return_start_dt is not None and return_start_dt < execution_dt:
        add_issue(findings, row_issues, "blocker", "return_starts_before_execution", "Forward-return window starts before execution timestamp.", row_number, entity, signal_key, {"execution_date": iso(execution_dt), "return_start_date": iso(return_start_dt)})

    if signal_dt is not None and execution_dt is not None and same_calendar_day(signal_dt, execution_dt) and (signal_date_only or execution_date_only):
        add_issue(findings, row_issues, args.same_day_execution_severity, "same_day_signal_execution_without_intraday_evidence", "Signal and execution are on the same date without intraday timestamp evidence.", row_number, entity, signal_key)
        if signal_execution_same_price is True:
            add_issue(findings, row_issues, args.same_day_execution_severity, "same_day_signal_execution_same_price", "Signal and execution use the same date and same price; this often indicates same-close execution bias.", row_number, entity, signal_key)
    if signal_dt is not None and return_start_dt is not None and same_calendar_day(signal_dt, return_start_dt) and (signal_date_only or return_start_date_only):
        add_issue(findings, row_issues, "warning", "same_day_signal_return_start", "Signal and forward-return start are on the same date; confirm the execution price and return-window convention.", row_number, entity, signal_key)
    if execution_dt is not None and return_start_dt is not None and same_calendar_day(execution_dt, return_start_dt) and (execution_date_only or return_start_date_only):
        if execution_return_start_same_price is True:
            pass
        elif execution_return_start_same_price is False:
            add_issue(findings, row_issues, "warning", "execution_return_start_price_mismatch", "Execution and return-start dates match but prices differ; confirm the return window starts after the actual execution price.", row_number, entity, signal_key)
        else:
            add_issue(findings, row_issues, "warning", "date_only_execution_return_start", "Execution and return-start dates match without intraday price convention evidence.", row_number, entity, signal_key)
    ...
    if signal_dt is not None and execution_dt is not None:
        ...
        if lag_days < args.min_signal_to_execution_lag_days:
            add_issue(findings, row_issues, args.signal_to_execution_lag_severity, "signal_to_execution_lag_too_short", "Signal-to-execution lag is shorter than the required minimum.", row_number, entity, signal_key, lag_days, args.min_signal_to_execution_lag_days)
        if args.max_signal_to_execution_lag_days is not None and lag_days > args.max_signal_to_execution_lag_days:
            add_issue(findings, row_issues, "warning", "signal_to_execution_lag_too_long", "Signal may be stale before execution.", row_number, entity, signal_key, lag_days, args.max_signal_to_execution_lag_days)
    if return_start_dt is not None and return_end_dt is not None:
        ...
        if horizon_days < args.min_return_horizon_days:
            add_issue(findings, row_issues, args.return_horizon_severity, "return_horizon_too_short", "Forward-return horizon is shorter than the required minimum.", row_number, entity, signal_key, horizon_days, args.min_return_horizon_days)
        if args.expected_horizon_days is not None and abs(horizon_days - args.expected_horizon_days) > args.horizon_tolerance_days:
            add_issue(findings, row_issues, "warning", "unexpected_return_horizon", "Forward-return horizon differs from the expected horizon.", row_number, entity, signal_key, horizon_days, args.expected_horizon_days)
    ...
    for label, value in [
        ...
        check_weekend(findings, row_issues, label, value, row_number, entity, signal_key, args.weekend_severity)
...
def build_report(header: list[str], rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    row_checks = [audit_row(header, row, row_number, args, findings) for row_number, row in enumerate(rows, start=1)]
    ...
    ranked = sorted(findings, key=lambda item: (severity_rank(item), item["check"], str(item.get("row_number", ""))), reverse=True)
    ...
    return {
        ...
        "problem_rows": sorted(problem_rows, key=lambda item: (-int(item["issue_count"]), str(item["entity"]), str(item["signal_date"])))[: args.finding_limit],
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if report["issue_counts"]:
        ...
        lines.append("- none")
    lines.extend(["", "## Top Findings", "", "| Severity | Check | Row | Entity | Signal date | Metric | Threshold | Detail |", "| --- | --- | --- | --- | --- | --- | --- | --- |"])
    if report["top_findings"]:
        ...
        lines.append("| info | no_findings |  |  |  |  |  | No execution-timing findings from supplied panel. |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit signal, execution, and forward-return timing alignment.")
    ...
    parser.add_argument("--portfolio-level", action="store_true", help="Treat rows as portfolio-level if entity column is absent.")
    ...
    parser.add_argument("--signal-to-execution-lag-severity", choices=["warning", "blocker"], default="blocker")
    ...
    parser.add_argument("--strict-missing-execution", action="store_true", help="Treat missing execution timestamps as blockers.")
    ...
    if args.min_signal_to_execution_lag_days < 0 or args.min_return_horizon_days < 0:
        raise SystemExit("--min-signal-to-execution-lag-days and --min-return-horizon-days must be non-negative.")
    if args.max_signal_to_execution_lag_days is not None and args.max_signal_to_execution_lag_days < args.min_signal_to_execution_lag_days:
