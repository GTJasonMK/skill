...
Standard-library only. This script checks whether research models, alpha
...
APPROVED_VALUES = {"approved", "pass", "passed", "signed_off", "complete", "completed", "yes", "y", "true", "1"}
...
RISK_RANK = {"low": 1, "medium": 2, "med": 2, "moderate": 2, "high": 3, "critical": 4, "tier_1": 4, "tier_2": 3, "tier_3": 2, "tier_4": 1}
...
def finding(severity: str, check: str, detail: str, model: str | None = None, metric: Any = None, threshold: Any = None) -> dict[str, Any]:
...
def check_columns(header: list[str], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    if missing:
        return [finding("warning", "missing_register_columns", "Model risk register is missing expected columns.", metric=missing)]
    return []


def evaluate_row(row: dict[str, str], index: int, args: argparse.Namespace, as_of: date) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if active and not text(row, args.owner_col):
        row_findings.append(finding("blocker" if live or high_risk else "warning", "missing_owner", "Active model has no owner.", mid))
    if active and rank == 0:
        row_findings.append(finding("blocker" if live else "warning", "missing_or_invalid_risk_tier", "Model has no recognized risk tier.", mid, tier or None))
    ...
    if active and (live or high_risk):
        if not validator:
            row_findings.append(finding("blocker", "missing_independent_validator", "Live or high-risk model has no independent validator.", mid))
        if not validation_status:
            row_findings.append(finding("blocker", "missing_validation_status", "Live or high-risk model has no validation status.", mid))
        elif validation_failed(validation_status) or not validation_ok(validation_status):
            row_findings.append(finding("blocker", "validation_not_approved", "Live or high-risk model validation is not approved.", mid, validation_status))
    elif active and validation_failed(validation_status):
        row_findings.append(finding("warning", "validation_failed", "Model validation is failed or blocked.", mid, validation_status))

    approval_status = text(row, args.approval_status_col)
    if live:
        if not approval_status:
            row_findings.append(finding("blocker", "missing_approval", "Live model has no approval status.", mid))
        elif not is_approved(approval_status):
            row_findings.append(finding("blocker", "approval_not_complete", "Live model is not approved for current status.", mid, approval_status))
    ...
    if active:
        if not last_review:
            row_findings.append(finding("warning" if not live else "blocker", "missing_last_review_date", "Active model has no parseable last review date.", mid, last_review_text or None))
        else:
            age = (as_of - last_review).days
            if age > max_age:
                row_findings.append(finding("blocker" if live or high_risk else "warning", "stale_model_review", "Model review is older than allowed for its risk tier.", mid, age, max_age))
        if not next_due:
            row_findings.append(finding("warning" if not live else "blocker", "missing_next_review_due", "Active model has no parseable next review due date.", mid, next_due_text or None))
        else:
            days_until_due = (next_due - as_of).days
            if days_until_due < 0:
                row_findings.append(finding("blocker" if live or high_risk else "warning", "review_overdue", "Model review due date has passed.", mid, days_until_due, 0))
            elif days_until_due <= args.warn_due_within_days:
                row_findings.append(finding("warning", "review_due_soon", "Model review is due soon.", mid, days_until_due, args.warn_due_within_days))
    ...
    for col, check, detail in [
        ...
        (args.kill_switch_col, "missing_kill_switch", "Live model has no kill-switch or manual override evidence."),
    ]:
    ...
    for col, check, detail in [
        ...
        (args.evidence_col, "missing_evidence_link", "Active model has no evidence link or artifact reference."),
    ]:
    ...
    if issue_count is not None and issue_count > args.max_open_issues:
        row_findings.append(finding("blocker" if live or high_risk else "warning", "open_issue_count", "Open model-risk issues exceed threshold.", mid, issue_count, args.max_open_issues))

    limitations = text(row, args.limitations_col)
    waiver = text(row, args.waiver_col)
    if active and limitations and not waiver:
        row_findings.append(finding("warning", "unwaived_limitations", "Model limitations are recorded without waiver or mitigation evidence.", mid))
...
def build_report(header: list[str], rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    ranked = sorted(findings, key=lambda item: (severity_rank(item), item.get("check", ""), item.get("model_id", "")), reverse=True)
    ...
    return {
        ...
        "models": sorted(models, key=lambda item: (item["blocker_count"], item["warning_count"], item["risk_rank"], item["model_id"]), reverse=True),
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Top Findings", "", "| Severity | Check | Model | Metric | Threshold | Detail |", "| --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit a quant model risk register for governance and review gaps.")
