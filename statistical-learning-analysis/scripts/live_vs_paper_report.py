...
Standard-library only. Input is a time-series CSV with date, live return, and
...
from quant_utils import correlation, parse_float, read_dataframe, require_columns, summarize_returns, summarize_values
        ...
        out.append(
            {
                ...
                "annualized_mean_gap": gap_summary["mean"] * annualization if gap_summary["mean"] is not None else None,
                "tracking_error": gap_summary["stdev"] * math.sqrt(annualization) if gap_summary["stdev"] is not None else None,
                "live_paper_correlation": correlation([item["live_return"] for item in subset], [item["paper_return"] for item in subset]),
...
def build_report(
        ...
        records.append(
            {
                ...
                "live_active_return": live_return - benchmark_return if benchmark_return is not None else None,
                "paper_active_return": paper_return - benchmark_return if benchmark_return is not None else None,
            }
        )
    ...
    paper_active = [item["paper_active_return"] for item in records if item["paper_active_return"] is not None]
    ...
    return {
        ...
        "benchmark_return_summary": summarize_returns(benchmark_returns, annualization, risk_free_annual) if benchmark_returns else None,
        "live_active_summary": summarize_returns(live_active, annualization, risk_free_annual) if live_active else None,
        "paper_active_summary": summarize_returns(paper_active, annualization, risk_free_annual) if paper_active else None,
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["rolling_gap"]:
        lines.append(f"| {item['end_date']} | {item['window']} | {item['mean_gap']} | {item['annualized_mean_gap']} | {item['tracking_error']} | {item['live_paper_correlation']} | {item['live_underperformed_periods']} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare live strategy returns with paper or backtest returns.")
    ...
    require_columns(header, [args.date_col, args.live_return_col, args.paper_return_col] + ([args.benchmark_return_col] if args.benchmark_return_col else []))
    report = build_report(rows, args.date_col, args.live_return_col, args.paper_return_col, args.benchmark_return_col, args.annualization, args.risk_free_annual, args.rolling_window)
    if args.output_json:
