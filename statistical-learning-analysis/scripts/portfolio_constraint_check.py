...
def build_report(
            ...
            for col, exposure in categories.items():
                for label, value in exposure.items():
                    if abs(value) > max_category_abs_weight:
                        violations.append({"constraint": f"max_abs_{col}", "category": label, "value": value, "limit": max_category_abs_weight})
        periods.append({
    ...
    return {
        ...
        "turnover_summary": summarize_series([item["turnover"] for item in periods if item["turnover"] is not None]),
        "periods": periods,
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["periods"]:
        ...
        lines.append(f"| {item['date']} | {item['passed']} | {item['gross_exposure']} | {item['net_exposure']} | {item['max_abs_weight']} | {item['turnover']} | {violations} |")
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check portfolio weights against common risk and trading constraints.")
    ...
    report = build_report(df, args.date_col, args.asset_col, args.weight_col, category_cols,
                         args.max_gross, args.min_net, args.max_net, args.max_abs_weight,
                         args.max_turnover, args.max_category_abs_weight)
    if args.output_json:
