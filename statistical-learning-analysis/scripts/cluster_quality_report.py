...
def build_report(
    ...
    cluster_sizes = {str(lbl): int(c) for lbl, c in zip(unique_labels, counts)}
    ...
    return {
        ...
        "notes": [
            ...
            "Bootstrap ARI measures the stability of k-means assignments to resampled data; low ARI suggests fragile clusters.",
        ],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cluster quality diagnostics with silhouette and bootstrap ARI stability.")
    ...
    parser.add_argument("--bootstrap", type=int, default=20, help="Bootstrap iterations for stability (only when k-means is used).")
    ...
    report = build_report(df, feature_cols, args.cluster_col, args.k, args.bootstrap,
                         args.sample_frac, not args.no_standardize, args.seed)
    if args.output_json:
