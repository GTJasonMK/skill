...
Standard-library only. This estimates a static hedge ratio and spread
...
def ols_y_on_x(y: list[float], x: list[float]) -> dict[str, float | None]:
    ...
    if var_x == 0:
        return {"intercept": None, "beta": None, "r2": None}
    beta = sum((xi - mx) * (yi - my) for xi, yi in zip(xx, yy)) / var_x
    intercept = my - beta * mx
    fitted = [intercept + beta * xi for xi in xx]
    sse = sum((yi - fi) ** 2 for yi, fi in zip(yy, fitted))
...
def count_crossings(zscores: list[float], threshold: float) -> dict[str, int]:
    ...
    for value in zscores[1:]:
        ...
        if (prev > 0 >= value) or (prev < 0 <= value):
            mean_reversions += 1
        prev = value
    return {"upper_threshold_crossings": upper, "lower_threshold_crossings": lower, "zero_crossings": mean_reversions}
...
def build_report(
    ...
    if intercept is not None and beta is not None:
        for idx, item in enumerate(observations, start=1):
            spread_path.append({"index": idx, "date": item["date"], "spread": item["a"] - intercept - beta * item["b"]})
    spreads = [item["spread"] for item in spread_path]
    spread_mean = mean(spreads)
    spread_sd = stdev(spreads)
    z_path = [
        {**item, "zscore": (item["spread"] - spread_mean) / spread_sd if spread_mean is not None and spread_sd not in {None, 0} else None}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Estimate pair-trading spread diagnostics from two price series.")
    ...
    report = build_report(rows, args.date_col, args.asset_a_col, args.asset_b_col, args.formation_window, args.log_prices, args.entry_z)
    if args.output_json:
