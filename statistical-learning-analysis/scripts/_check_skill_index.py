...
def parse_skill_md() -> tuple[set[str], set[str], set[str]]:
    ...
    script_mentions = set(re.findall(r"`?scripts/([A-Za-z0-9_]+\.py)`?", text))
    reference_mentions = set(re.findall(r"references/([A-Za-z0-9_\-]+\.md)", text))
    ...
    if references_section:
        scripts_in_references_section = set(re.findall(r"scripts/([A-Za-z0-9_]+\.py)", references_section.group(1)))
        references_in_references_section = set(re.findall(r"references/([A-Za-z0-9_\-]+\.md)", references_section.group(1)))
    return (
...
def main() -> int:
    ...
    if missing_in_refs:
        errors.append("Missing from SKILL.md ## References section (script links): " + ", ".join(sorted(missing_in_refs)))
    ...
    if missing_md_in_skill:
        errors.append("Missing from SKILL.md ## References section (doc links): " + ", ".join(sorted(missing_md_in_skill)))
    ...
    if missing_in_imap:
        errors.append("Missing from references/implementation-map.md Bundled Scripts table: " + ", ".join(sorted(missing_in_imap)))
