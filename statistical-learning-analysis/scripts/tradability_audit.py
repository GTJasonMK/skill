...
Standard-library only. This script complements point-in-time and execution
...
def bool_value(value: str | None, true_terms: set[str] | None = None, false_terms: set[str] | None = None) -> bool | None:
    ...
    for col in [args.order_qty_col, args.trade_value_col, args.target_weight_col]:
        if present(header, col):
            ...
            if value is not None and abs(value) > args.min_abs_trade:
                return True
    return not any(present(header, col) for col in [args.order_qty_col, args.trade_value_col, args.target_weight_col])
...
def limit_flags(row: dict[str, str], header: list[str], args: argparse.Namespace) -> tuple[bool | None, bool | None]:
    up = bool_value(text(row, args.limit_up_col), {"up", "locked", "limit", "limit_up", "limitup"}, {"open", "active", "normal", "none"}) if present(header, args.limit_up_col) else None
    down = bool_value(text(row, args.limit_down_col), {"down", "locked", "limit", "limit_down", "limitdown"}, {"open", "active", "normal", "none"}) if present(header, args.limit_down_col) else None
...
def participation(row: dict[str, str], header: list[str], args: argparse.Namespace) -> tuple[float | None, str]:
    ...
    trade_value = parse_float(text(row, args.trade_value_col)) if present(header, args.trade_value_col) else None
    dollar_volume = parse_float(text(row, args.dollar_volume_col)) if present(header, args.dollar_volume_col) else None
...
def audit_row(
    ...
    if not present(header, args.date_col):
        add_issue(findings, row_issues, "warning", "missing_date_column", "No trade or evaluation date column is present.", row_number, entity, None)
    elif date_dt is None:
        add_issue(findings, row_issues, "warning", "invalid_or_missing_date", "Date is missing or not parseable as an ISO-style date/timestamp.", row_number, entity, None, text(row, args.date_col))
    ...
    execution_price = parse_float(text(row, args.execution_price_col)) if present(header, args.execution_price_col) else None
    close_price = parse_float(text(row, args.close_price_col)) if present(header, args.close_price_col) else None
    tradable = bool_value(text(row, args.tradable_col), {"tradable", "open", "active", "available", "normal"}, {"not_tradable", "halted", "suspended", "closed", "unavailable"}) if present(header, args.tradable_col) else None
    halted = bool_value(text(row, args.halted_col), {"halted", "halt", "stopped"}, {"open", "active", "normal", "tradable"}) if present(header, args.halted_col) else None
    suspended = bool_value(text(row, args.suspended_col), {"suspended", "suspend"}, {"open", "active", "normal", "tradable"}) if present(header, args.suspended_col) else None
    shortable = bool_value(text(row, args.shortable_col), {"shortable", "available", "borrowable"}, {"not_shortable", "unavailable", "restricted"}) if present(header, args.shortable_col) else None
    borrow_available = bool_value(text(row, args.borrow_available_col), {"available", "borrowable", "located"}, {"unavailable", "none", "no_borrow", "not_available"}) if present(header, args.borrow_available_col) else None
    borrow_rate = parse_float(text(row, args.borrow_rate_col)) if present(header, args.borrow_rate_col) else None
    limit_up, limit_down = limit_flags(row, header, args)
    part, part_basis = participation(row, header, args)

    if tradable is False:
        add_issue(findings, row_issues, "blocker", "not_tradable_flag", "Tradable flag is false on a row used for the strategy.", row_number, entity, date_key)
    if halted is True:
        add_issue(findings, row_issues, "blocker", "halted_security", "Security is marked halted on the trade/evaluation date.", row_number, entity, date_key)
    if suspended is True:
        add_issue(findings, row_issues, "blocker", "suspended_security", "Security is marked suspended on the trade/evaluation date.", row_number, entity, date_key)

    if has_trade and present(header, args.execution_price_col):
        if execution_price is None or execution_price <= 0:
            add_issue(findings, row_issues, "blocker", "missing_or_nonpositive_execution_price", "Execution price is missing or nonpositive for a traded row.", row_number, entity, date_key, execution_price)
    elif has_trade and not present(header, args.execution_price_col):
        add_issue(findings, row_issues, "warning", "missing_execution_price_column", "No execution price column is present; tradability evidence is incomplete.", row_number, entity, date_key)

    if present(header, args.close_price_col) and close_price is not None and close_price <= 0:
        add_issue(findings, row_issues, "warning", "nonpositive_close_price", "Close price is nonpositive; market data may be invalid.", row_number, entity, date_key, close_price)

    if present(header, args.volume_col):
        if volume is None:
            add_issue(findings, row_issues, "warning", "missing_volume", "Volume is missing; liquidity and zero-volume checks are incomplete.", row_number, entity, date_key)
        elif volume <= args.min_volume:
            severity = "blocker" if has_trade or args.strict_zero_volume else "warning"
            add_issue(findings, row_issues, severity, "zero_or_tiny_volume", "Volume is zero or below the minimum tradable volume threshold.", row_number, entity, date_key, volume, args.min_volume)
    elif has_trade:
        add_issue(findings, row_issues, "warning", "missing_volume_column", "No volume column is present; liquidity evidence is incomplete.", row_number, entity, date_key)

    if present(header, args.adv_col) and adv is not None and adv <= args.min_adv:
        add_issue(findings, row_issues, "warning", "zero_or_tiny_adv", "ADV is zero or below the minimum ADV threshold.", row_number, entity, date_key, adv, args.min_adv)

    if has_trade and part is not None and part > args.max_participation:
        add_issue(findings, row_issues, args.participation_severity, "participation_too_high", f"Trade participation is above threshold using {part_basis}.", row_number, entity, date_key, part, args.max_participation)
    if has_trade and part is None and not any(present(header, col) for col in [args.volume_col, args.dollar_volume_col, args.adv_col]):
        add_issue(findings, row_issues, "warning", "missing_participation_evidence_gap", "No share or dollar volume evidence is available to estimate participation.", row_number, entity, date_key)

    if side == "buy" and limit_up is True:
        add_issue(findings, row_issues, "blocker", "buy_at_limit_up", "Buy order is simulated while the asset is limit-up locked.", row_number, entity, date_key)
    if side == "sell" and limit_down is True:
        add_issue(findings, row_issues, "blocker", "sell_at_limit_down", "Sell order is simulated while the asset is limit-down locked.", row_number, entity, date_key)
    if side is None and (limit_up is True or limit_down is True):
        add_issue(findings, row_issues, "warning", "limit_lock_without_side", "Asset has a limit-lock flag but trade side is unavailable.", row_number, entity, date_key)

    if side == "sell":
        if shortable is False:
            add_issue(findings, row_issues, "blocker", "short_not_allowed", "Sell/short row is not shortable according to the shortable flag.", row_number, entity, date_key)
        if borrow_available is False:
            add_issue(findings, row_issues, "blocker", "borrow_unavailable", "Sell/short row lacks borrow availability.", row_number, entity, date_key)
        if borrow_rate is not None and borrow_rate > args.max_borrow_rate:
            add_issue(findings, row_issues, "warning", "borrow_rate_too_high", "Borrow rate is above threshold and may invalidate net economics.", row_number, entity, date_key, borrow_rate, args.max_borrow_rate)
        if args.require_short_evidence and not any(present(header, col) for col in [args.shortable_col, args.borrow_available_col]):
            add_issue(findings, row_issues, "warning", "missing_short_evidence_gap", "No shortable or borrow-availability column is present for a sell/short row.", row_number, entity, date_key)

    if not any(present(header, col) for col in [args.tradable_col, args.halted_col, args.suspended_col, args.volume_col, args.limit_status_col, args.limit_up_col, args.limit_down_col]):
        add_issue(findings, row_issues, "warning", "missing_market_state_evidence_gap", "No tradable/halt/suspension/volume/limit evidence columns are present.", row_number, entity, date_key)
...
def stale_price_findings(row_checks: list[dict[str, Any]], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for entity, rows in by_entity.items():
        ...
        for item in ordered:
            ...
            if isinstance(price, (int, float)) and last_price is not None and abs(float(price) - float(last_price)) <= args.stale_price_tolerance:
                streak.append(item)
            else:
                streak = [item]
            last_price = float(price) if isinstance(price, (int, float)) else None
            if len(streak) >= args.stale_price_days and all((row.get("volume") or 0) <= args.min_volume for row in streak):
                first = streak[0]
                out.append(
                    finding(
                        "warning",
                        ...
                        {"rows": [row["row_number"] for row in streak[-args.stale_price_days :]], "price": last_price},
...
def build_report(header: list[str], rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "problem_rows": sorted(problem_rows, key=lambda item: (-int(item["issue_count"]), str(item["entity"]), str(item.get("date", ""))))[: args.finding_limit],
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if report["issue_counts"]:
        ...
        lines.append("- none")
    lines.extend(["", "## Top Findings", "", "| Severity | Check | Row | Entity | Date | Metric | Threshold | Detail |", "| --- | --- | --- | --- | --- | --- | --- | --- |"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit tradability and market-state evidence for factor, backtest, or portfolio rows.")
    ...
    parser.add_argument("--require-short-evidence", action="store_true", help="Warn when sell/short rows lack shortable or borrow columns.")
    parser.add_argument("--strict-zero-volume", action="store_true", help="Treat zero/tiny volume as a blocker even when no trade-size column is present.")
