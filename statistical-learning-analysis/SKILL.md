...
- **Core (required for all bundled scripts):** `pandas>=2.0`, `numpy>=1.24`, `scipy>=1.10`. Install via `pip install -r requirements.txt`.
- **Optional:** `scikit-learn>=1.3` (required only by `sklearn_tabular_model.py` and `cluster_quality_report.py`) and `joblib>=1.3` (model persistence in `sklearn_tabular_model.py`). Install via `pip install -r requirements-optional.txt`.
- **Development validation:** `PyYAML>=6.0` for the upstream skill quick validator. Install via `pip install -r requirements-dev.txt`.
- A few stdlib-only utilities (`profile_dataset.py`, `split_dataset.py`, `missingness_report.py`, `panel_summary.py`, `causal_balance_check.py`, `time_series_backtest.py`, `classification_report.py`, `threshold_tuning.py`, `compare_model_reports.py`) still run on the standard library alone.
...
- `scripts/profile_dataset.py`: Profile CSV structure, column types, missingness, imbalance, time/group hints, and leakage risk flags. Standard-library only.
- `scripts/split_dataset.py`: Create train/test CSV splits using random, stratified, time-ordered, or grouped strategies. Standard-library only.
- `scripts/causal_balance_check.py`: Compute standardized mean-difference covariate balance diagnostics for treated/control data. Standard-library only.
- `scripts/time_series_backtest.py`: Backtest naive and seasonal-naive forecasting baselines by horizon. Standard-library only.
...
- `scripts/classification_report.py`: Evaluate classification predictions from labels and/or positive-class scores. Standard-library only.
- `scripts/threshold_tuning.py`: Tune binary classification thresholds for F1, precision, recall, specificity, or simple cost. Standard-library only.
- `scripts/missingness_report.py`: Summarize missingness by column, row, and missingness pattern. Standard-library only.
- `scripts/panel_summary.py`: Summarize entity/time panel structure and repeated-observation risks. Standard-library only.
- `scripts/compare_model_reports.py`: Compare JSON model reports into a metric leaderboard. Standard-library only.
...
- `scripts/returns_risk_report.py`: Compute return/risk metrics, drawdowns, VaR/ES, Sharpe/Sortino, and correlations for return or price CSVs. Standard-library only.
- `scripts/factor_exposure_regression.py`: Regress asset/strategy returns on factor returns and report alpha, betas, t-stats, R-squared, and residual risk. Standard-library only.
- `scripts/point_in_time_audit.py`: Audit date-entity signal panels for point-in-time availability, look-ahead leakage, universe timing, revisions, execution ordering, and duplicate as-of keys. Standard-library only.
- `scripts/execution_timing_audit.py`: Audit signal, rebalance, execution, and forward-return window timing for same-day execution gaps, non-executable return windows, stale signals, weekend dates, and duplicate signal keys. Standard-library only.
- `scripts/tradability_audit.py`: Audit market-state tradability evidence for halted/suspended assets, zero volume, limit locks, high participation, stale prices, shorting, and borrow availability. Standard-library only.
- `scripts/factor_ic_report.py`: Compute per-date cross-sectional IC and rank IC for factor values against forward returns. Standard-library only.
- `scripts/factor_quantile_report.py`: Evaluate factor-sorted quantile forward returns and high-minus-low spreads. Standard-library only.
- `scripts/factor_decay_report.py`: Compare IC and rank IC across multiple forward-return horizons. Standard-library only.
- `scripts/factor_turnover_report.py`: Estimate selected-name turnover, membership overlap, and rank autocorrelation across rebalance dates. Standard-library only.
- `scripts/signal_overlap_report.py`: Diagnose correlation, rank correlation, selected-name overlap, and redundancy across multiple alpha signal columns. Standard-library only.
- `scripts/incremental_alpha_report.py`: Test whether a candidate alpha adds residual IC, coefficient, and R-squared value after existing signals or exposures. Standard-library only.
- `scripts/portfolio_backtest.py`: Backtest date-asset portfolio weights against asset returns with gross/net metrics and turnover. Standard-library only.
- `scripts/transaction_cost_report.py`: Estimate turnover-driven commissions, slippage, spread, borrow, cost drag, and optional ADV participation. Standard-library only.
- `scripts/covariance_report.py`: Compute covariance, annualized covariance, correlation, and volatility diagnostics for return columns. Standard-library only.
- `scripts/ewma_volatility.py`: Compute EWMA volatility paths and latest annualized volatility for return columns. Standard-library only.
- `scripts/rolling_beta.py`: Estimate rolling alpha, beta, R-squared, and residual volatility against a benchmark. Standard-library only.
- `scripts/pairs_spread_report.py`: Estimate static hedge ratio, spread z-scores, crossings, autocorrelation, and approximate half-life for a pair. Standard-library only.
- `scripts/event_study_report.py`: Compute simple benchmark-adjusted or mean-adjusted event-study abnormal returns and CAR. Standard-library only.
- `scripts/cross_sectional_return_regression.py`: Run single-date or pooled cross-sectional return regressions on asset characteristics/exposures. Standard-library only.
- `scripts/fama_macbeth_regression.py`: Run date-by-date Fama-MacBeth regressions and summarize average risk premia. Standard-library only.
- `scripts/long_short_backtest.py`: Form signal-ranked long/short or long-only portfolios and report gross/net performance and turnover. Standard-library only.
- `scripts/portfolio_exposure_report.py`: Aggregate numeric and categorical portfolio exposures from date-asset weights. Standard-library only.
- `scripts/pca_risk_model.py`: Compute PCA statistical risk-factor diagnostics from return covariance/correlation matrices. Standard-library only.
- `scripts/factor_neutralization.py`: Residualize a factor signal against numeric and categorical exposures within each date. Standard-library only.
- `scripts/newey_west_regression.py`: Run OLS with IID and Newey-West/HAC standard errors for time-series regressions. Standard-library only.
- `scripts/multiple_testing_report.py`: Apply Bonferroni, Holm, and Benjamini-Hochberg corrections to factor/model p-values. Standard-library only.
- `scripts/quant_experiment_audit.py`: Audit a quant research experiment registry for unregistered trials, missing failed variants, selective promotion, final-test gaps, and version evidence. Standard-library only.
- `scripts/capacity_impact_report.py`: Estimate ADV participation, simple market impact, cost bps, and binding NAV capacity from weight changes. Standard-library only.
- `scripts/portfolio_constraint_check.py`: Check portfolio weights against gross, net, single-name, category, and turnover constraints. Standard-library only.
- `scripts/bootstrap_reality_check.py`: Run a block-bootstrap reality-check diagnostic across many strategy return columns. Standard-library only.
- `scripts/alpha_research_gate_report.py`: Gate candidate alpha research using bundled JSON diagnostics for experiment audit, IC, incremental value, overlap, turnover, costs, capacity, and multiple testing. Standard-library only.
- `scripts/walk_forward_stability.py`: Evaluate walk-forward parameter-selection stability from date/parameter/metric results. Standard-library only.
- `scripts/optimizer_sensitivity_report.py`: Assess mean-variance weight sensitivity to expected-return and covariance perturbations. Standard-library only.
- `scripts/portfolio_construction_gate_report.py`: Gate portfolio construction using bundled JSON diagnostics for backtest, constraints, exposures, risk contribution, optimizer stability, costs, and capacity. Standard-library only.
- `scripts/performance_attribution_report.py`: Attribute portfolio return to assets and optional groups from weights and asset returns. Standard-library only.
- `scripts/risk_contribution_report.py`: Compute portfolio volatility and component risk contributions from weights and covariance. Standard-library only.
- `scripts/regime_robustness_report.py`: Evaluate return robustness across user-provided market regimes. Standard-library only.
- `scripts/risk_forecast_calibration.py`: Check volatility and normal-VaR forecast calibration against realized returns. Standard-library only.
- `scripts/model_risk_register_report.py`: Audit quant model-risk registers for owners, risk tiers, validation, approvals, review dates, monitoring, rollback controls, and version evidence. Standard-library only.
- `scripts/execution_slippage_report.py`: Summarize side-aware execution slippage and implementation shortfall from fills. Standard-library only.
- `scripts/live_vs_paper_report.py`: Compare live strategy returns with paper or backtest returns. Standard-library only.
- `scripts/signal_health_monitor.py`: Monitor factor or alpha-signal coverage, IC, spread, and turnover health. Standard-library only.
- `scripts/go_live_gate_report.py`: Summarize quant strategy go-live checklist status and blockers. Standard-library only.
- `scripts/order_exception_report.py`: Summarize rejected, cancelled, open, and partially filled order exceptions. Standard-library only.
- `scripts/data_freshness_report.py`: Check dataset timestamp, row-count, missingness, and upstream status freshness. Standard-library only.
- `scripts/limit_breach_report.py`: Summarize risk, portfolio, and operations limit breaches. Standard-library only.
- `scripts/strategy_action_decision.py`: Convert monitoring thresholds into maintain/review/reduce/pause/retire action recommendations. Standard-library only.
- `scripts/quant_checklist_template.py`: Generate default go-live, monitoring, or retirement checklist templates. Standard-library only.
- `scripts/quant_report_aggregator.py`: Aggregate bundled quant JSON diagnostics into one strategy review or production health report. Standard-library only.
- `scripts/quant_review_pack.py`: Generate committee-style quant review packs from bundled JSON diagnostics with decision stack, role review, evidence gaps, and next actions. Standard-library only.
