...
Requires the shared bundle core dependencies. Input is a dataset monitoring CSV with dataset name,
...
def build_report(
    ...
    for row in rows:
        ...
        if not dataset:
            ...
            continue
        max_age = (
            max_age_value if max_age_value is not None and max_age_value > 0 else default_max_age_minutes
        )
        age_minutes = (now - timestamp).total_seconds() / 60 if timestamp is not None else None
        missing_rate = (
            missing_count / row_count if missing_count is not None and row_count not in {None, 0} else None
        )
        issues = []
    ...
    return {
        ...
        "notes": [
            ...
            "A fresh timestamp does not prove the data is correct; pair this with "
            "missingness and reconciliation checks.",
        ],
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Dataset Checks",
            "",
            "| Dataset | Age minutes | Max age | Row count | Missing rate | Status | Issues |",
            "| --- | --- | --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["checks"]:
        lines.append(
            f"| {item['dataset']} | {item['age_minutes']} | {item['max_age_minutes']} | "
            f"{item['row_count']} | {item['missing_rate']} | {item['upstream_status']} | "
            f"{', '.join(item['issues']) if item['issues'] else 'none'} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check data freshness, row-count, and missingness health by dataset."
    )
    ...
    optional = [
        col
        for col in [args.max_age_minutes_col, args.row_count_col, args.missing_count_col, args.status_col]
        if col
    ]
