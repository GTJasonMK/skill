...
def build_report(
    ...
    return {
        ...
        "long_only": long_only,
        "seed": seed,
        "base_weights": {asset: float(weight) for asset, weight in zip(assets, base_weights, strict=False)},
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["weight_summary"]:
        lines.append(
            f"| {item['asset']} | {item['base_weight']} | {item['mean_weight']} | {item['stdev_weight']} | {item['min_weight']} | {item['max_weight']} | {item['sign_flip_rate']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Assess simple mean-variance weight sensitivity to input perturbations."
    )
