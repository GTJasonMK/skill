...
Requires the shared bundle core dependencies. Input is a time-series CSV with realized returns and a
...
def build_report(
    ...
    hit_autocorr = (
        correlation([float(v) for v in hits[:-1]], [float(v) for v in hits[1:]]) if len(hits) > 2 else None
    )
        ...
        calibration_bins.append(
            {
                ...
                "realized_to_forecast_rms": bin_realized / bin_forecast_rms
                if bin_realized is not None and bin_forecast_rms not in {None, 0}
                else None,
    ...
    return {
        ...
        "realized_to_forecast_rms": realized_rms / forecast_rms
        if realized_rms is not None and forecast_rms not in {None, 0}
        else None,
...
def main() -> int:
    ...
    report = build_report(
        ...
        args.date_col,
        args.return_col,
        args.forecast_vol_col,
        args.annualization,
        args.forecast_vol_annualized,
        args.var_level,
        args.bins,
    )
    if args.output_json:
