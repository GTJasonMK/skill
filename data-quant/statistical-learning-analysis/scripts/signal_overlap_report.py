...
Requires the shared bundle core dependencies. Input is a long date-asset CSV with several signal
...
def build_report(
    ...
    for date in sorted(by_date):
        ...
        for i, left in enumerate(signal_cols):
            for right in signal_cols[i + 1 :]:
                common_assets = sorted(
                    {asset for asset, _ in date_signals[left]} & {asset for asset, _ in date_signals[right]}
                )
                ...
                pair = {
                    ...
                    "pearson": correlation(left_values, right_values)
                    if len(common_assets) >= min_pair_count
                    else None,
        ...
        per_date.append(
            {"date": date, "asset_count": len(assets), "signals": signal_counts, "pairs": date_pairs}
        )
    ...
    for key, items in sorted(pair_rows.items()):
        ...
        redundant = (mean_abs_spearman is not None and mean_abs_spearman >= corr_threshold) or (
            mean_overlap is not None and mean_overlap >= overlap_threshold
        )
        pair_summary.append(
            {
                "pair": key,
                ...
                "high_abs_spearman_rate": sum(abs(value) >= corr_threshold for value in spearman_values)
                / len(spearman_values)
                if spearman_values
                else None,
                "high_overlap_rate": sum(value >= overlap_threshold for value in overlap_values)
                / len(overlap_values)
                if overlap_values
                else None,
                "redundant": redundant,
    ...
    for signal, items in sorted(signal_rows.items()):
        ...
        redundancy_partners = [
            item for item in pair_summary if item["redundant"] and signal in {item["left"], item["right"]}
        ]
        overlap_partners = []
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["redundant_pairs"]:
        lines.append(
            f"| {item['pair']} | {item['dates']} | {item['abs_spearman_summary']['mean']} | {item['top_overlap_summary']['mean']} | {item['high_abs_spearman_rate']} | {item['high_overlap_rate']} |"
        )
    lines.extend(
        [
            "",
            "## All Pairs",
            "",
            "| Pair | Mean Pearson | Mean Rank Corr | Mean Top Overlap | Mean Jaccard | Redundant |",
            "| --- | --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["pair_summary"]:
        lines.append(
            f"| {item['pair']} | {item['pearson_summary']['mean']} | {item['spearman_summary']['mean']} | {item['top_overlap_summary']['mean']} | {item['jaccard_summary']['mean']} | {item['redundant']} |"
        )
    lines.extend(
        [
            "",
            "## Signal Summary",
            "",
            "| Signal | Mean coverage | Mean selected | Redundant partners | Mean pair overlap |",
            "| --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["signal_summary"]:
        lines.append(
            f"| {item['signal']} | {item['coverage_summary']['mean']} | {item['selected_count_summary']['mean']} | {item['redundant_partner_count']} | {item['mean_pair_overlap']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Diagnose overlap and redundancy across multiple alpha signal columns."
    )
