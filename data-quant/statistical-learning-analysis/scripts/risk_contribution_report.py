...
def _read_covariance(
    df: pd.DataFrame, asset_col: str, other_asset_col: str, cov_col: str, assets: list[str]
) -> np.ndarray:
    df = df.copy()
...
def build_report(
    ...
    rows = []
    for asset, weight, marginal in zip(assets, weights, cov_w, strict=False):
        ...
        rows.append(
            {
                ...
                "annualized_component_volatility_contribution": pct * annualized_volatility
                if pct is not None
                else None,
            }
        )
    return {
        ...
        "risk_contributions": sorted(
            rows, key=lambda item: abs(item["percent_variance_contribution"] or 0.0), reverse=True
        ),
        "notes": [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute portfolio volatility and component risk contributions."
    )
