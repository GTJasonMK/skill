...
Requires the shared bundle core dependencies. Input is a metric CSV with date, metric, observed value,
...
def build_report(
    ...
    return {
        ...
        "date_breach_counts": {
            date: sum(item["is_breach"] for item in by_date[date])
            for date in sorted_group_keys(list(by_date))
        },
        "severity_breach_counts": {
            severity: sum(item["is_breach"] for item in items)
            for severity, items in sorted(by_severity.items())
        },
        "strategy_breach_counts": {
            strategy: sum(item["is_breach"] for item in items)
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for metric, item in report["metric_summary"].items():
        lines.append(
            f"| {metric} | {item['observations']} | {item['breach_count']} | {item['breach_rate']} | {item['max_consecutive_breaches']} |"
        )
    lines.extend(
        [
            "",
            "## Blockers",
            "",
            "| Date | Metric | Value | Limit | Direction | Severity | Owner |",
            "| --- | --- | --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["blockers"]:
        lines.append(
            f"| {item['date']} | {item['metric']} | {item['value']} | {item['limit']} | {item['direction']} | {item['severity']} | {item['owner']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    ...
    optional = [
        col for col in [args.direction_col, args.severity_col, args.owner_col, args.strategy_col] if col
    ]
