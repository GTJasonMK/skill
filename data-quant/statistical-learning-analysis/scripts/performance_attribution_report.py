        ...
        periods.append(
            {
                ...
                "group_contribution": {
                    col: {k: float(v) for k, v in by_group[col].items()} for col in group_cols
                },
            }
        )
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["asset_contribution_summary"]:
        lines.append(
            f"| {item['asset']} | {item['total_contribution']} | {item['mean']} | {item['t_stat']} | {item['positive_rate']} |"
        )
    for col, items in report["group_contribution_summary"].items():
        lines.extend(
            [
                ...
                f"## {col} Contribution",
                "",
                "| Group | Total contribution | Mean | t-stat |",
                "| --- | --- | --- | --- |",
            ]
        )
        for item in items:
            lines.append(
                f"| {item['group']} | {item['total_contribution']} | {item['mean']} | {item['t_stat']} |"
            )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Attribute portfolio return to assets and optional groups from weights."
    )
    ...
    report = build_report(
        df, args.date_col, args.asset_col, args.weight_col, args.return_col, group_cols, args.annualization
    )
    if args.output_json:
