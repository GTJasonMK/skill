...
def per_class_metrics(y_true: list[str], y_pred: list[str], labels: list[str]) -> list[dict[str, Any]]:
    ...
    for label in labels:
        ...
        tn = sum(1 for t, p in zip(y_true, y_pred, strict=False) if t != label and p != label)
...
def macro_weighted(metrics: list[dict[str, Any]]) -> dict[str, float | None]:
    ...
    def weighted(key: str) -> float | None:
        ...
        if not vals:
            return None
        return sum(v * w for v, w in zip(vals, weights, strict=False)) / sum(weights)
...
def accuracy_score(y_true: list[str], y_pred: list[str]) -> float:
    return sum(t == p for t, p in zip(y_true, y_pred, strict=False)) / len(y_true) if y_true else float("nan")
...
def roc_auc_score(y_true: list[str], scores: list[float], positive_label: str) -> float | None:
    ...
    sum_pos = sum(rank for rank, label in zip(ranks, labels, strict=False) if label == 1)
...
def binary_loss(y_true: list[str], probs: list[float], positive_label: str) -> dict[str, float | None]:
    ...
    brier = sum((p - y) ** 2 for p, y in zip(clipped, labels, strict=False)) / len(labels) if labels else None
    log_loss = (
        -sum(y * math.log(p) + (1 - y) * math.log(1 - p) for p, y in zip(clipped, labels, strict=False))
        / len(labels)
        if labels
        else None
    )
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Per Class",
            "",
            "| Label | Support | Precision | Recall | Specificity | F1 |",
            "| --- | --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["per_class"]:
        lines.append(
            f"| {item['label']} | {item['support']} | {item['precision']} | "
            f"{item['recall']} | {item['specificity']} | {item['f1']} |"
        )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.5, help="Threshold for converting scores to labels."
    )
    ...
    report = classification_report(
        rows, args.truth, args.prediction, args.score, args.positive_label, args.threshold
    )
    if args.output_json:
