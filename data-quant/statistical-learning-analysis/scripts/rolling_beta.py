...
def _regression_stats(y: np.ndarray, x: np.ndarray, annualization: int) -> dict[str, Any]:
    ...
    blank = {
        ...
        "beta": None,
        "alpha": None,
        "annualized_alpha_arithmetic": None,
        "r2": None,
        "residual_volatility": None,
        "annualized_residual_volatility": None,
    }
    if n < 2:
        return blank
    yy = y[:n]
    ...
    return {
        ...
        "annualized_residual_volatility": resid_vol * np.sqrt(annualization)
        if resid_vol is not None
        else None,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Estimate rolling beta and alpha of a return series versus a benchmark."
    )
    ...
    require_columns(
        df,
        [args.return_col, args.benchmark_col]
