...
def infer_decision(data: Any) -> str:
    if not isinstance(data, dict):
        return "review"
    for field in ["decision", "recommended_action", "gate_decision", "register_decision", "audit_decision"]:
...
def append_items(findings: list[dict[str, Any]], source: str, kind: str, items: Any, limit: int) -> None:
    ...
    for item in items[:limit]:
        if isinstance(item, dict):
            ...
            severity = "info"
        findings.append(
            {"source": source, "kind": kind, "severity": severity, "name": name, "detail": detail}
        )
...
def report_findings(data: Any, source: str, limit: int) -> list[dict[str, Any]]:
        ...
        findings.append(
            {
                ...
                "detail": "Report JSON root is not an object.",
            }
        )
        return findings
    for field, kind in [
    ...
    for count_field, kind in [
        ...
        if value > 0:
            findings.append(
                {"source": source, "kind": kind, "severity": "info", "name": count_field, "detail": value}
            )
...
def summarize_reports(paths: list[Path], finding_limit: int) -> dict[str, Any]:
    ...
    ranked_findings = sorted(
        all_findings,
        key=lambda item: (severity_rank(item.get("severity")), item.get("source", ""), item.get("kind", "")),
        reverse=True,
    )
    return {
...
def markdown(report: dict[str, Any], title: str) -> str:
    ...
    if report["decision_counts"]:
        for decision, count in sorted(
            report["decision_counts"].items(), key=lambda item: (-DECISION_RANK.get(item[0], 0), item[0])
        ):
    ...
    lines.extend(
        [
            ...
            "## Reports",
            "",
            "| Report | Type | Decision | Key metrics | Findings |",
            "| --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["reports"]:
        ...
        lines.append(
            f"| {md_escape(item['name'])} | {md_escape(item['type'])} | {md_escape(item['decision'])} | {md_escape(metrics)} | {item['finding_count']} |"
        )
    ...
    lines.extend(
        [
            ...
            "## Top Findings",
            "",
            "| Source | Kind | Severity | Name | Detail |",
            "| --- | --- | --- | --- | --- |",
        ]
    )
    for item in report["top_findings"]:
        lines.append(
            f"| {md_escape(item['source'])} | {md_escape(item['kind'])} | {md_escape(item['severity'])} | {md_escape(item['name'])} | {md_escape(item['detail'])} |"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Aggregate bundled quant JSON diagnostics into one review report."
    )
