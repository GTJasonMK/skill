...
Requires the shared bundle core dependencies. Input is an order or fill CSV with date, asset, order
        ...
        alerts.append(
            {
                ...
                "detail": f"Exception rate above {max_exception_rate}.",
            }
        )
    if (
        overall["aggregate_fill_rate"] is not None
        and overall["aggregate_fill_rate"] < min_aggregate_fill_rate
    ):
        alerts.append(
            {
                "severity": "warning",
                "name": "low_aggregate_fill_rate",
                "detail": f"Aggregate fill rate below {min_aggregate_fill_rate}.",
            }
        )
    rejected = overall["status_counts"].get("rejected", 0)
    if rejected:
        alerts.append(
            {"severity": "warning", "name": "rejected_orders", "detail": f"{rejected} rejected orders found."}
        )
...
def markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## By Date",
            "",
            "| Date | Orders | Exception rate | Aggregate fill rate |",
            "| --- | --- | --- | --- |",
        ]
    )
    for date, item in report["by_date"].items():
        lines.append(
            f"| {date} | {item['orders']} | {item['exception_rate']} | {item['aggregate_fill_rate']} |"
        )
    lines.extend(
        [
            "",
            "## By Asset",
            "",
            "| Asset | Orders | Exception rate | Aggregate fill rate |",
            "| --- | --- | --- | --- |",
        ]
    )
    for asset, item in report["by_asset"].items():
        lines.append(
            f"| {asset} | {item['orders']} | {item['exception_rate']} | {item['aggregate_fill_rate']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Summarize order exceptions, fill quality, and rejection/partial-fill rates."
    )
    ...
    require_columns(
        header,
        [args.date_col, args.asset_col, args.status_col, args.quantity_col, args.filled_quantity_col]
        + optional,
    )
    report = build_report(
        rows,
