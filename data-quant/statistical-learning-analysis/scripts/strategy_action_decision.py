...
Requires the shared bundle core dependencies. Input is a CSV with metric names, observed values,
...
def build_report(
    ...
    if triggered_rules:
        ...
        recommended_action = sorted(strongest, key=lambda rule: (rule["category"], rule["metric"]))[0][
            "action"
        ]
    else:
    ...
    return {
        ...
        "triggered_rules": sorted(
            triggered_rules, key=lambda rule: (-rule["action_rank"], rule["category"], rule["metric"])
        ),
        "rules": rules,
        "notes": [
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if report["triggered_by_action"]:
        for action, count in sorted(
            report["triggered_by_action"].items(), key=lambda item: (-ACTION_RANK.get(item[0], 0), item[0])
        ):
    ...
    lines.extend(
        [
            ...
            "## Triggered Rules",
            "",
            "| Action | Category | Metric | Value | Threshold | Direction | Margin | Owner | Reason |",
            "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        ]
    )
    for rule in report["triggered_rules"]:
        lines.append(
            f"| {rule['action']} | {rule['category']} | {rule['metric']} | {rule['value']} | {rule['threshold']} | {rule['direction']} | {rule['margin']} | {rule['owner']} | {rule['reason']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Turn monitoring metrics into strategy action recommendations."
    )
    ...
    optional = [
        col
        for col in [args.direction_col, args.category_col, args.reason_col, args.owner_col]
