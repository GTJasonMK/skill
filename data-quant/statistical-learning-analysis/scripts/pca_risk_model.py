...
Requires the shared bundle core dependencies. Uses a Jacobi eigenvalue algorithm for symmetric
...
def covariance_matrix(data: list[list[float]]) -> list[list[float]]:
    ...
    return [
        [sum((row[i] - means[i]) * (row[j] - means[j]) for row in data) / (n - 1) for j in range(p)]
        for i in range(p)
    ]
...
def correlation_matrix(data: list[list[float]]) -> list[list[float]]:
    ...
    return [
        [
            cov[i][j] / (vols[i] * vols[j]) if vols[i] > 0 and vols[j] > 0 else (1.0 if i == j else 0.0)
            for j in range(p)
        ]
        for i in range(p)
    ]
...
def jacobi_eigen(
    matrix: list[list[float]], max_iter: int = 200, tol: float = 1e-12
...
def build_report(
    rows: list[dict[str, str]], columns: list[str], matrix_type: str, components: int
) -> dict[str, Any]:
    data = []
    dropped = 0
    for row in rows:
    ...
    pairs = sorted(zip(eigenvalues, eigenvectors, strict=False), key=lambda item: item[0], reverse=True)
        ...
        component_rows.append(
            {
                ...
                "loadings": {col: loading for col, loading in zip(columns, vector, strict=False)},
            }
        )
    asset_stats = []
    ...
    return {
        ...
        "matrix": {
            row_name: {col_name: matrix[i][j] for j, col_name in enumerate(columns)}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute a simple PCA risk-model diagnostic from return columns."
    )
