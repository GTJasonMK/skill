...
Requires the shared bundle core dependencies. Supports Bonferroni, Holm, and Benjamini-Hochberg FDR
...
def build_report(rows: list[dict[str, str]], p_col: str, id_col: str | None, alpha: float) -> dict[str, Any]:
    ...
    output = []
    for item, p_bonf, p_holm, p_bh in zip(tests, bonf, holm_adj, bh, strict=False):
        output.append(
            {
                "id": item["id"],
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["tests"][:20]:
        lines.append(
            f"| {item['id']} | {item['p_value']} | {item['bonferroni_p']} | {item['holm_p']} | {item['bh_fdr_p']} | {item['significant_bh_fdr']} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply multiple-testing corrections to factor or model p-values."
    )
