...
def build_design(
    df: pd.DataFrame, return_col: str, factor_cols: list[str], risk_free_col: str | None
...
def run_regression(
    df: pd.DataFrame, return_col: str, factor_cols: list[str], risk_free_col: str | None, annualization: int
) -> dict[str, Any]:
    ...
    for name, coef, se, t_stat in zip(
        names, result["coefficients"], result["standard_errors_iid"], result["t_stats_iid"], strict=False
    ):
        row = {
            "name": name,
            "coefficient": coef,
    ...
    result.update(
        {
            ...
            "notes": [
                ...
                "Standard errors and t-stats are IID OLS approximations; use HAC/"
                "Newey-West for autocorrelated financial returns.",
            ],
        }
    )
    return result
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["coefficient_table"]:
        ...
        lines.append(
            f"| {row['name']} | {row['coefficient']} | "
            f"{row['standard_error_iid']} | {row['t_stat_iid']} | "
            f"{annual_alpha} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Estimate alpha and factor betas from return and factor columns."
    )
    ...
    parser.add_argument(
        "--risk-free-col", help="Optional risk-free return column subtracted from dependent returns."
    )
    parser.add_argument(
        "--annualization", type=int, default=252, help="Periods per year for annualized alpha."
    )
