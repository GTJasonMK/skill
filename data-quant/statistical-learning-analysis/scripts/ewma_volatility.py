...
def build_report(
    ...
    for col in columns:
        ...
        for idx, (date, ret, v, vol_i, ann_i) in enumerate(
            zip(dates, rets, variance, vol, ann_vol, strict=False), start=1
        ):
            path.append(
                {
                    "index": idx,
        ...
        assets.append(
            {
                ...
                "sample_annualized_volatility": sample_vol * np.sqrt(annualization)
...
def markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Asset | N | Latest EWMA vol | Latest ann. EWMA vol | Mean ann. EWMA vol | "
        "Sample ann. vol | Rows dropped |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for item in report["assets"]:
        lines.append(
            f"| {item['asset']} | {item['n']} | {item['latest_ewma_volatility']} | "
            f"{item['latest_annualized_ewma_volatility']} | "
            f"{item['mean_annualized_ewma_volatility']} | "
            f"{item['sample_annualized_volatility']} | {item['rows_dropped']} |"
        )
    lines.extend(["", "Notes:"])
