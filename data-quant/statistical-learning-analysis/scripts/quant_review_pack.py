...
def finding_from_item(
    source: str, report_type: str, kind: str, item: Any, default_severity: str
) -> dict[str, Any]:
...
def extract_findings(data: Any, source: str, report_type: str, limit: int) -> list[dict[str, Any]]:
    if not isinstance(data, dict):
        return [
            finding_from_item(source, report_type, "load", "Report JSON root is not an object.", "blocker")
        ]
            ...
            if value > 0:
                ...
                findings.append(
                    finding_from_item(
                        ...
                        report_type,
                        count_field,
                        {"check": count_field, "detail": value, "severity": severity},
                        severity,
                    )
                )
...
def roles_for_finding(report_type: str, item: dict[str, Any]) -> list[str]:
    ...
    if base:
        roles.add(base)
    text = " ".join(
        str(item.get(key, "")) for key in ["check", "detail", "kind", "source", "report_type"]
    ).lower()
    ...
    for item in findings:
        ...
        for role in item.get("roles", []):
            ...
            if len(summary[role]["findings"]) < 5:
                summary[role]["findings"].append(
                    {"source": item["source"], "check": item["check"], "severity": severity}
                )
    return summary
...
def next_actions(report: dict[str, Any]) -> list[str]:
    ...
    if report["overall_decision"] in {"fail", "pause", "stop", "retire"}:
        actions.append(
            "Do not promote or scale until blocker findings are resolved and source diagnostics are regenerated."
        )
    elif report["overall_decision"] in {"review", "warn", "warning", "reduce", "de-risk"}:
        actions.append(
            "Hold reviewer sign-off until warning findings and evidence gaps have explicit owners or waivers."
        )
    else:
        actions.append(
            "Record reviewer sign-off, preserve source diagnostics, and define monitoring thresholds before the next stage."
        )
    for role in ROLE_ORDER:
        row = report["role_summary"][role]
        if row["blockers"] or row["warnings"]:
            actions.append(
                f"{role}: resolve {row['blockers']} blocker(s) and {row['warnings']} warning(s), or document approved waivers."
            )
    if not actions:
...
def build_report(paths: list[Path], args: argparse.Namespace) -> dict[str, Any]:
    ...
    ranked_findings = sorted(
        findings,
        key=lambda item: (severity_rank(item.get("severity")), item.get("source", ""), item.get("check", "")),
        reverse=True,
    )
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["reports"]:
        ...
        lines.append(
            f"| {md_escape(item['name'])} | {md_escape(item['type'])} | {md_escape(item['decision'])} | {md_escape(metrics)} | {md_escape(roles)} |"
        )
    ...
    lines.extend(
        [
            ...
            "## Role Review",
            "",
            "| Role | Blockers | Warnings | Representative issues |",
            "| --- | --- | --- | --- |",
        ]
    )
    for role in ROLE_ORDER:
    ...
    lines.extend(
        [
            ...
            "## Top Findings",
            "",
            "| Severity | Roles | Source | Check | Detail | Metric | Threshold |",
            "| --- | --- | --- | --- | --- | --- | --- |",
        ]
    )
    ...
    if report["evidence_gaps"]:
        for item in report["evidence_gaps"]:
            lines.append(
                f"| {md_escape(item['source'])} | {md_escape(item['check'])} | {md_escape(item.get('detail'))} |"
            )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a committee-style quant review pack from bundled JSON diagnostics."
    )
