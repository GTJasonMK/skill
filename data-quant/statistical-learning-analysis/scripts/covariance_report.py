...
def build_report(df: pd.DataFrame, columns: list[str], annualization: int) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "These are historical estimates; shrinkage, regime checks, and out-of-sample "
            "realized-risk validation are needed before optimization.",
        ],
    }
...
def matrix_markdown(title: str, matrix: dict[str, dict[str, Any]], columns: list[str]) -> list[str]:
    ...
    for row in columns:
        lines.append(
            ...
            + row
            + " | "
            + " | ".join(str(matrix[row][col]) for col in columns)
            + " |"
        )
    return lines
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["assets"]:
        lines.append(
            f"| {item['asset']} | {item['n']} | {item['mean_return']} | "
            f"{item['annualized_return_arithmetic']} | {item['volatility']} | "
            f"{item['annualized_volatility']} | {item['rows_dropped']} |"
        )
    lines.extend([""])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute covariance, correlation, and volatility diagnostics for return columns."
    )
