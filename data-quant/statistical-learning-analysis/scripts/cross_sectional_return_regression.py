...
def build_report(
    ...
    y, X, skipped_by_date, dropped_missing = build_design(
        df, return_col, feature_cols, date_col, date_value, intercept
    )
    try:
        fit = ols(y, X)
    ...
    for name, coef, se, t_stat in zip(
        names, fit["coefficients"], fit["standard_errors_iid"], fit["t_stats_iid"], strict=False
    ):
        row = {
            "name": name,
            "coefficient": coef,
...
def markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["coefficient_table"]:
        lines.append(
            f"| {row['name']} | {row['coefficient']} | {row['standard_error_iid']} | "
            f"{row['t_stat_iid']} | {row.get('annualized_intercept_arithmetic', '')} |"
        )
    lines.extend(["", "Notes:"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a cross-sectional return regression from asset-level rows."
    )
    ...
    report = build_report(
        df, args.return_col, feature_cols, args.date_col, args.date, not args.no_intercept, args.annualization
    )
    if args.output_json:
