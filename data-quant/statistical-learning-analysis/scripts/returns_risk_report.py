...
def summarize_returns_extended(
    returns: pd.Series, annualization: int, risk_free_annual: float, var_level: float
) -> dict[str, Any]:
    ...
    ann_ret_geom = (
        compounded ** (annualization / n) - 1.0 if compounded is not None and compounded > 0 and n else None
    )
    ...
    sharpe = (
        (avg_excess / vol) * np.sqrt(annualization)
        if avg_excess is not None and vol not in {None, 0} and vol is not None and vol > 0
        else None
    )
    sortino = (
        (avg_excess / downside_dev) * np.sqrt(annualization)
        if avg_excess is not None
        and downside_dev not in {None, 0}
        and downside_dev is not None
        and downside_dev > 0
        else None
    )
...
def build_report(
    ...
    corr = {
        a: {b: (None if pd.isna(corr_df.at[a, b]) else float(corr_df.at[a, b])) for b in columns}
        for a in columns
    }
    return {
        "mode": mode,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute return and risk diagnostics from return or price CSV columns."
    )
