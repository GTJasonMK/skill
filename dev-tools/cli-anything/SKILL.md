...
4. If the installed resources are unavailable and this skill is being used from a
   `CLI-Anything` repository checkout, read `bundle/cli-anything-plugin/HARNESS.md` and
   the resources around it.
5. If a bundled file is missing, report the missing file instead of creating an external dependency.
6. Only if all full-methodology sources are unavailable, follow the condensed rules below.

The installer vendors the canonical resources from the bundled `cli-anything-plugin/` into this
